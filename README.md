...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Under the canonical GDD, Content Production Spec, and Development Roadmap, commits `fb8f56a`, `9095cf7`, `c22ebbe`, and `6faf3bd` define, load, and carry the versioned canonical death/destruction/Memorial/Echo terminal signature through hosted PostgreSQL, exact replay, authenticated real-QUIC response loss, and a newly launched `server_app` process. Hosted CI `29480575194` proves byte-identical signatures and death views across those checkpoints, with strict lint/tests/content checks and the Windows release build green. The active [`GB-M03-06E`](docs/tasks/GB-M03-06E.md) step is the missing/altered/duplicate summary/Memorial/destruction/Echo/cleanup/outbox matrix, concurrent lethal writers, participant rollback/crash boundaries, performance, cleanup, and the joint `06`/`02D`/`13` audits. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
