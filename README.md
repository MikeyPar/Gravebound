...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schemas `0043`–`0048` define normalized live evidence, v2 deed receipts, crash-time deed revocation, the contract-1 authoritative clock command, and retained live-trace ingestion. Commits `895b445` and `57ec9a6` add a serialized predecessor-head contract and the server-owned trace service: one start-or-resume path reconstructs the exact locked character version and retained 300-tick window, dynamic simulation/durable identities remain monotonic and bounded, deterministic rejection requires reload, ambiguous commit loss retries the sealed request without recreating inputs, and lethal evidence stays isolated with a complete paired-identity projection for the future death transaction. Local full persistence/server tests and strict all-target Clippy are green. The earlier hosted crash-restore fixture failures are corrected by `df4f6d2`/`1bf1866`; exact-head CI `29413531719` is running. Next obtain that hosted proof, implement additive migration `0049` and make the live-trace promotion envelope mandatory inside the durable death/Echo transaction, then prove coordinator response-loss/restart/rollback through PostgreSQL and real QUIC. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
