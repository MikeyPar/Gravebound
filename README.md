...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md) slice now has schemas `0031`-`0033`: normalized death/summary/memorial/trace/Echo/material records, strict replay/ledger/ownership/history bindings, and a component-complete danger-entry restore V2. Accepted [`SPEC-CONFLICT-027`](docs/spec-conflicts/SPEC-CONFLICT-027-m03-crash-restore-completeness.md) records why exact recovery also requires a forward-only V3 baseline for pre-entry Backpack items, consumed Belt identities, Bargain provenance, and compensating Ash authority. Require exact-head hosted proof for schema 33, then publish migration `0034` plus V3 capture/restore DTOs and adverse fixtures before implementing the strict death repository/protocol. Keep all player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal-route behavior fail closed.
