...
Commits `83ccbb1`, `83c75a5`, `3f4ecaf`, and `8a4898d` implement the durable B3 authority, corrected terminal semantics, transport-independent executor, and canonical persistent loader. Commit `3c05a36` removes the B6 tick-rewind blocker by starting Caldus scheduling, body, health, and hostile projectiles on the inherited danger tick while preserving phase-local cadence. Next consume the committed B5 boss-staging handoff into one route-bound Caldus owner and drive exact staging, countdown, introduction, and combat phases. Bound normal-server construction, hosted B3 inactivity/restart proof, boss rewards/pending inventory, stable exit, and normal admission remain disabled.
