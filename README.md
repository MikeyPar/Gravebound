...
[`GB-M03-03F`](docs/milestones/GB-M03-03F-audit.md) is complete under its three-authority audit. Strict transition copy, the sequence-safe native loading/error/LinkLost/reconnect projection, the optimized 33-frame standard/reduced-effects matrix, layered adverse recovery, and the disposable real-QUIC Character Select -> Hall -> private life -> Caldus -> Hall route all pass without opening normal ingress. Mandatory PostgreSQL completed 25 scripted loops with 25.620 ms median login-to-control (29.874 ms p95, 37.903 ms max). The exact optimized executable passed a 30-minute 1920x1080 soak at p95 8.423 ms, p99 11.420 ms, and 338.3 MB peak RSS with no monotonic leak. Continue the parent `GB-M03-03` roadmap audit, `GB-M03-04E`-`04G`, and the approved `SPEC-CONFLICT-009` death foundation next. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
