...
Commits `f5c12b3`, `42a632b`, and `be52c29` complete the worker-owned pending-inventory publication seam, reject impossible/non-Core custody, and add exact rollback for uncommitted extraction reservations. The worker publishes pending inventory before `BossExitReady`, retains both events, and replays them on each newer transport generation; exact danger authority can also clear a registered extraction owner during `LinkLost`. See the [extraction-prerequisite evidence](docs/evidence/GB-M03-03G-caldus-extraction-prerequisites-evidence.md). Next construct/register the production extraction actor from the exact committed exit and storage version vector, bind it through the transport-independent session owner, and prove active/reconnected/`LinkLost` and competing-terminal behavior. Then run hosted PostgreSQL/real-QUIC adverse and restart proof. Bound normal-server construction and normal admission remain disabled.
