...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **69% complete**: identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete Core item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, and successful extraction/Emergency Recall/ResolutionHold are closed under hosted evidence. [`GB-M03-08`](docs/milestones/GB-M03-08-audit.md) passed at commit `2535e8c` on hosted CI [`29554811453`](https://github.com/MikeyPar/Gravebound/actions/runs/29554811453). Successor recovery has append-only protocol `1.17`, reliable kind `23`, `core_successor_v1`, pinned `1.16` compatibility, migration `0060`, and ordinary-death preset/reservation writes green through the hosted mandatory PostgreSQL gate; a pre-existing-roster correction for the supplemental Echo branch fixture and the atomic successor-creation writer remain next. Normal route admission, telemetry, support, hosting/platform evidence, and final private-loop/cohort gates remain disabled.
...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Publish the supplemental death-branch fixture correction that pre-creates its second living roster identity before the predecessor death reserves successor recovery, then require the complete hosted run to stay green. The exact `0060` writer is already green through the mandatory PostgreSQL gate; after the corrected evidence generator passes, extract the transaction-local `StarterItemPlan` seam and implement the serializable successor-creation writer. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
