...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md) slice has the mandatory five-component V3 capture and immutable normalized crash-result graph in schema `0034`, terminal request/replay/conflict authority in `0035`, and the forward-only typed-world-revision trigger repair in `0036`. Hosted schema-34 proof applied the migration and then exposed that published trigger's stale pre-`0005` column reference; exact schema-36 run `29382451012` is now the authoritative hosted gate. Accepted [`SPEC-CONFLICT-027`](docs/spec-conflicts/SPEC-CONFLICT-027-m03-crash-restore-completeness.md) defines completeness, and correction [`SPEC-CONFLICT-028`](docs/spec-conflicts/SPEC-CONFLICT-028-m03-crash-baseline-identity-capacity.md) fixes the legal maximum at 64 durable identities without changing slot or stack capacities. Complete and prove the single serializable all-component crash-restore repository next, then start the strict death repository/protocol. Keep all player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal-route behavior fail closed.
