...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. Under the canonical GDD, Content Production Spec, and Development Roadmap, the active [`GB-M03-06`](docs/tasks/GB-M03-06.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) backend has one serializable death/destruction/Memorial/Echo transaction, retained-live-trace promotion, five-producer terminal precedence, seven participant rollback injections, response-loss/process-restart replay, and real PostgreSQL-pool outage/reconnect convergence. Catalog commit `0b39ec1` and authority commit `ac062c1` now keep dangerous-world provenance separate from immutable death-presentation hashes through additive schema `0051`, bind protocol `1.14`/death-view schema `2` latest, summary, Memorial, and trace responses to the stored presentation revision, reject unsupported historical packages as typed content mismatch, and validate source/attack/pattern/status producers before trace persistence or resume. Hosted catalog run `29436521860` and exact authority run `29442803974` are green across Windows release, PostgreSQL migrations/transactions, formatting, strict lint, tests, content, deterministic trace, and schemas. Next build the renderer-independent durable-ack summary and Memorial list/detail models, then prove local health zero and an unacknowledged terminal commit cannot expose losses or successor actions before connecting the native UI. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
