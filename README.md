...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schemas `0043`–`0045` define normalized live `06B` clocks, deed receipts, and the bounded ten-second trace graph. The additive v2 deed contract derives exact Sir Caldus/Sepulcher authority from committed reward and XP evidence, binds the danger root and life-metrics version, audits changed payloads, pairs crash revocation with projection rebuilding, and preserves Sepulcher as a final deed without falsely satisfying `ECH-001`. The internal terminal coordinator requires all five server producers to evaluate a tick before sealing and preserves death's absolute same-tick precedence. Next implement and host-prove the v2 deed transaction, then complete clock/trace repositories and server ingestion, wire the coordinator to execution/recovery, and prove committed death through real QUIC with response loss/restart. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
