...
Commits `83ccbb1`, `83c75a5`, and `3f4ecaf` implement the durable B3 reward/progression/milestone authority, correct its terminal semantics, and add the transport-independent automatic executor: malformed authority fails closed, SOC-010 ineligibility commits no grant, exact acknowledgement never re-pends, progression precedes route publication, transient database errors retry without transport, and the retained result replays contiguously to each newer reliable-writer generation; see the [three-authority evidence](docs/evidence/GB-M03-03G-durable-b3-reward-authority-evidence.md). Next inject the PostgreSQL coordinator with a process-bound `SecretRewardEpoch` at production route construction and prove hosted inactivity zero-row, response-loss, reconnect, and process-restart convergence. Then complete B5 and authoritative Sir Caldus B6. Normal admission remains disabled.
