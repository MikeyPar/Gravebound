...
Commits through `ca0953d` add the dedicated `BoundCorePrivateLifeServer`, terminal-first generation-safe attach, decoded domain dispatch before acknowledgement, Hall/danger datagram routing, dynamic Recall/extraction authorities, monotonic shared-writer route events, and the native `core-private-life` Character Select/Play/Hall route. Capability publication now occurs only from the complete persistent server/client composition; the older identity endpoint remains isolated. The Current Next Step is to publish live driver observations and bounded gameplay snapshots without duplicate route versions, reconcile stored extraction/Recall/death outcomes into the exact Hall/summary/successor client state, and close the remaining interaction dispatch. Then implement telemetry, support lookup, and hosting/platform evidence before the user-deferred cumulative audit and external cohort/Steamworks gates.
