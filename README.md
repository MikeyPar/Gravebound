...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **97% complete**. Identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, extraction/Emergency Recall/ResolutionHold, and successor recovery are closed under hosted evidence. The active private-route integration now owns clocks, deeds, damage evidence, death planning, atomic death/Echo execution, one coherent current-danger snapshot, and a live post-Caldus terminal heartbeat; lethal health, extraction, explicit Recall, and LinkLost disconnect recovery now enter the same route-bound terminal coordinator. Remaining work is verified-fault restoration, ordinary dispatch, telemetry/support/hosting, and final hosted/private-cohort/platform gates.
...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282). Commits `8cd626e`, `50d858a`, `5d9dc33`, `1ec0285`, `fa44c81`, and `8c4c907` extend the [durable terminal-owner evidence](docs/evidence/GB-M03-03G-durable-terminal-owner-evidence.md): one serializable PostgreSQL read supplies all terminal versions, custody, clocks, counts, and active root; lethal health, extraction, explicit Recall, and LinkLost disconnect recovery arbitrate before acknowledgement; Caldus exit-ready continues neutral 30 Hz terminal ticks; accepted extraction planning retries without changing its durable tick; and only executor-issued proof can publish the winning extraction. Next add the typed verified-fault restoration producer and its exact crash-restore receipt. Normal admission remains fail closed until all five producers share this coordinator and focused restart/shutdown proof passes.
