...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Under the canonical GDD, Content Production Spec, and Development Roadmap, commits `fb8f56a` through `6faf3bd` define, load, replay, reconnect, and process-restart the versioned canonical death/destruction/Memorial/Echo terminal signature. Commits `9ac2c39` through `d302b1c` prove fail-closed altered summary, Memorial, destruction, Echo, Bargain-cleanup, and death-outbox reconstruction and add migration `0052`'s append-only character-life outbox. Commit `e16424e` closes the remaining causal-outbox cardinality gap: PostgreSQL rejects a duplicate `death_committed` row by its named unique constraint, the canonical reader rejects a missing row, and exact restoration returns identical bytes. Hosted CI `29485778651` is green for PostgreSQL 17, strict lint/tests/content/schema/determinism checks, and the optimized Windows build. The active [`GB-M03-06E`](docs/tasks/GB-M03-06E.md) step is measured durable-commit-to-interactive/query latency, task/session/transaction/lock cleanup, stable memory, eligibility/promotion branch coverage, and the joint `06`/`02D`/`13` audits. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
