...
Commits `3c05a36`, `cdeceda`, `8efcec0`, `b61cbce`, `4eba061`, `c433cbd`, and `f7f7fbb` now carry B5 authority into a route-bound B6 owner: exact compiled arena, player/combat/projectile identity, inherited 30 Hz tick, five-second countdown, immutable participant lock, resumable 75-tick introduction, same-tick Phase 1 encounter, legal early-phase defeat projection, stale-route rollback, and separate `0.70` physical-body collision for walking/Slipstep are locally green. Commit `7669e41` supplies the separately verified unregistered transition pack. Next implement the approved post-charge de-overlap rule and run the complete deterministic fight through defeat before composing committed boss reward/pending-inventory placement and the stable exit. Bound normal-server construction, hosted B3 adverse/restart proof, and normal admission remain disabled.
