...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Atomic death/Memorial/Echo has real-QUIC/PostgreSQL restart, replay, adverse, latency, soak, reachable-branch, and distinct-account concurrency evidence. The server-owned `06C` planner converts raw custody into exact Equipment, Belt, stacked RunBackpack, PersonalGround, and run-material destruction; [`SPEC-CONFLICT-028`](docs/spec-conflicts/SPEC-CONFLICT-028-m03-runbackpack-unit-destruction-order.md) records the stacked-unit tie-break. Hosted run `29500484938` correctly rejected a CharacterSafe fixture inserted before its inventory owner; that ordering is repaired. A 17-family matrix now attempts 34 production-triggered UPDATE/DELETE mutations, and a separate production-service composition test retries item, progression, Bargain, and world commands after committed death. Transaction-scoped dead-character outcomes prevent posthumous receipts while the database triggers remain the final race barrier; both tests require unchanged row counts, terminal signature, and exact death replay. Death closure remains open for hosted acceptance and optimized native capture. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
