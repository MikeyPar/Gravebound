...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **74% complete**: identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete Core item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, and successful extraction/Emergency Recall/ResolutionHold are closed under hosted evidence. [`GB-M03-08`](docs/milestones/GB-M03-08-audit.md) passed at commit `2535e8c` on hosted CI [`29554811453`](https://github.com/MikeyPar/Gravebound/actions/runs/29554811453). Successor recovery has append-only protocol `1.17`, reliable kind `23`, `core_successor_v1`, pinned `1.16` compatibility, migration `0060`, death-side preset/reservation authority green through hosted CI [`29571991103`](https://github.com/MikeyPar/Gravebound/actions/runs/29571991103), the shared transaction-local `04D` starter seam, a replay-first atomic successor writer that checks stored mutation authority before fresh content admission, and an authenticated fail-closed service. Hosted run [`29585161647`](https://github.com/MikeyPar/Gravebound/actions/runs/29585161647) is fully green for migration `0061`, all 11 rollback boundaries, concurrent fresh/replay, process/pool restart, conflict audit, complete selected successor/starter graph, strict Linux quality, optimized Windows release, and supplemental native evidence. Reliable successor dispatch, deliberate response-loss bot support, and an explicitly disabled normal runtime are locally green through all 255 server tests, all-target Clippy, and the compiled QUIC suite. Persistent disposable real-QUIC/restart proof is next. Normal route admission, telemetry, support, hosting/platform evidence, and final private-loop/cohort gates remain disabled.
...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Hosted run [`29585161647`](https://github.com/MikeyPar/Gravebound/actions/runs/29585161647) closes the atomic writer/service database gate. Exercise the newly injected successor authority through disposable real QUIC against PostgreSQL: deliberate response loss, exact replay after reconnect and process/pool restart, altered-payload conflict, feature-absent normal endpoint, malformed/oversized input, and zero transport/task/connection/database residue. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
