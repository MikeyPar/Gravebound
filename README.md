...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06`](docs/tasks/GB-M03-06.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) backend now has one serializable death/destruction/Memorial/Echo transaction, retained-live-trace promotion, five-producer terminal precedence, seven participant rollback injections, response-loss/process-restart replay, and real PostgreSQL-pool outage/reconnect convergence. Hosted CI runs `29431899916`, `29432319053`, and exact documentation head `29432451470` are green. Commit `0b39ec1` begins [`GB-M03-06D`](docs/tasks/GB-M03-06D.md) with a dedicated transitive death-presentation catalog whose typed resolvers fail closed across class, source, attack/pattern, status, item, Oath, Bargain, and stored UI copy; its workspace gates are green and hosted run `29436521860` is in progress. Next bind durable death records and readers to that catalog revision, canonicalize producer IDs, then build the renderer-independent durable-ack summary and Memorial models. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
