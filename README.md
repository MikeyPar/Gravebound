...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schemas `0043`–`0045` define normalized live `06B` clocks, deed receipts, and the bounded ten-second trace graph. The serializable v2 deed repository now derives exact Sir Caldus/Sepulcher authority from locked terminal reward and XP evidence, binds the selected living danger character, immutable entry root, active lineage, promoted content revisions, character version, and life-metrics CAS, records canonical receipts and changed-payload audits, and rejects legacy identity collisions or a divergent latest-deed projection. A comprehensive hosted PostgreSQL adverse fixture is registered in CI; exact-head hosted execution is still required before closure. Next obtain that hosted proof, then implement canonical crash revocation/projection rebuilding and complete clock/trace persistence and server ingestion. Afterward wire the five-producer terminal coordinator to execution/recovery and prove committed death through real QUIC with response loss and restart. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
