...
> **Project status:** M01 and M02 are closed under their recorded gates. M03 has closed identity, parent PostgreSQL persistence, exact world-flow content, atomic dormant transfers, Hall/private-microrealm simulation and native evidence, progression, the complete Core 18-item/equipment/CharacterSafe/Vault lifecycle, the first Oath/Bargain package, the minimal Ash wallet, and atomic death/destruction/Memorial/Echo persistence with hosted adverse, outage, response-loss, restart, soak, and native evidence. Parents `GB-M03-02`, `GB-M03-04`, and `GB-M03-06` are complete. `GB-M03-08` now has hosted-green append-only protocol `1.16`: extraction/Recall `1.15` bytes remain unchanged while bounded ResolutionHold query/mutation kinds `21`/`22` are appended under their own disabled capability. Terminal custody `0055`, atomic extraction graph `0056`, authenticated extraction dispatch, Recall graph `0057` with deterministic custody and immutable recovery, replay identity `0058`, exact server-owned 12-tick explicit/90-tick LinkLost coordination, a bounded per-character mailbox, and the staged five-producer terminal driver are implemented. The disposable active route is hosted green for Hall-to-danger Recall commit, lost completion, pool/actor reconstruction, and exact replay. ResolutionHold migration `0059` adds the immutable whole-stack result/transition/audit/outbox graph, retained extraction provenance and Overflow deadline authority, final Hall unlock versions, request-hash conflict binding, and fail-closed transaction/immutability guards. Hosted run `29539028461` proved the migration applied, then failed only because its exact table-inventory fixture omitted the five new tables; commit `f4e26ea` corrects that fixture and fresh hosted validation is pending. Commits `23f331d`, `0def341`, and `1ce0bb1` add the pure whole-stack planner, bounded durable Hold domain, and serializable read authority with canonical hashes, exact authored capacity, nonrecursive replay verification, immutable extraction-provenance checks, Hall/security binding, and server-owned destination previews. ResolutionHold mutation writer/service/UI work, remaining adverse/cleanup evidence, successor recovery, complete-route admission, telemetry, support, platform evidence, and final private-loop/cohort gates remain disabled.
...
Parents [`GB-M03-02`](docs/milestones/GB-M03-02-audit.md), [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and [`GB-M03-06`](docs/milestones/GB-M03-06-audit.md), plus atomic Echo package [`GB-M03-13`](docs/milestones/GB-M03-13-audit.md), are complete. Hosted CI [`29506273492`](https://github.com/MikeyPar/Gravebound/actions/runs/29506273492) is the final death/Echo acceptance. [`GB-M03-08`](docs/tasks/GB-M03-08.md) preserves old protocol bytes while appending extraction/Recall kinds `19`/`20`; extraction custody/writer/recovery/dispatch are hosted green through [`29521716146`](https://github.com/MikeyPar/Gravebound/actions/runs/29521716146), durable Recall replay identity through [`29531220086`](https://github.com/MikeyPar/Gravebound/actions/runs/29531220086), the bounded actor mailbox through [`29531939971`](https://github.com/MikeyPar/Gravebound/actions/runs/29531939971), the immutable full-snapshot driver through [`29534531389`](https://github.com/MikeyPar/Gravebound/actions/runs/29534531389), and the real PostgreSQL lost-completion/pool-and-actor restart route through [`29536980393`](https://github.com/MikeyPar/Gravebound/actions/runs/29536980393). Accepted [`SPEC-CONFLICT-030`](docs/spec-conflicts/SPEC-CONFLICT-030-m03-resolution-hold-recovery.md) is implemented at the wire boundary by commit `265d50e`; protocol `1.16` preserves kinds `1`-`20` and appends bounded Hold kinds `21`/`22`, proven by hosted run [`29537777576`](https://github.com/MikeyPar/Gravebound/actions/runs/29537777576). Commit `f0efde1` adds migration `0059`: immutable replay result, whole-stack UID transitions, request-hash conflict audit, unpublished outbox, retained extraction/Overflow authority, exact aggregate advances, final Hall unlock, and transaction/immutability guards. Hosted run [`29539028461`](https://github.com/MikeyPar/Gravebound/actions/runs/29539028461) applied `0059` and failed only when the exact public-table fixture omitted its five new tables; commit `f4e26ea` corrects that inventory and fresh hosted validation is pending. Commits `23f331d`, `0def341`, and `1ce0bb1` add the deterministic Hold planner, bounded persistence domain, and serializable current-Hold read with canonical stack/request/nonrecursive-result hashes, immutable extraction provenance, exact authored stack validation, Hall/security authority, and server-owned destination previews. Next, obtain green hosted schema proof and implement the replay-first serializable Move/DestroyConfirmed writer with exact conflict, rollback, concurrency, and restart tests; then add service/dispatch and the native resolver. In parallel, close remaining Recall adverse/cleanup evidence. After `GB-M03-08`, complete `GB-M03-07` successor recovery and parent `GB-M03-03`; keep normal route admission and Core promotion fail closed.
