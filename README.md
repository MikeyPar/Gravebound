...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **72% complete**: identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete Core item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, and successful extraction/Emergency Recall/ResolutionHold are closed under hosted evidence. [`GB-M03-08`](docs/milestones/GB-M03-08-audit.md) passed at commit `2535e8c` on hosted CI [`29554811453`](https://github.com/MikeyPar/Gravebound/actions/runs/29554811453). Successor recovery has append-only protocol `1.17`, reliable kind `23`, `core_successor_v1`, pinned `1.16` compatibility, migration `0060`, death-side preset/reservation authority green through hosted CI [`29571991103`](https://github.com/MikeyPar/Gravebound/actions/runs/29571991103), the shared transaction-local `04D` starter seam, a replay-first atomic successor writer that checks stored mutation authority before fresh content admission, and an authenticated fail-closed service locally green through all 255 server tests and strict all-target Clippy. Hosted runs [`29579576357`](https://github.com/MikeyPar/Gravebound/actions/runs/29579576357) and [`29581539059`](https://github.com/MikeyPar/Gravebound/actions/runs/29581539059) successively exposed a duplicate migration-`0033` initializer and a relation-specific `NEW.published_at` reference in migration `0060`; both are repaired. Run [`29583478039`](https://github.com/MikeyPar/Gravebound/actions/runs/29583478039) then cleared migration `0061` and all 11 injected rollback boundaries before its post-commit verifier decoded `character_progression.current_health` (`INT4`) as `i16`; the verifier now uses `i32`, with production authority unchanged. The corrected hosted PostgreSQL run is next. Normal route admission, telemetry, support, hosting/platform evidence, and final private-loop/cohort gates remain disabled.
...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Re-run the mandatory hosted `postgres_successor` gate with the corrected `INT4` health verifier. Run [`29583478039`](https://github.com/MikeyPar/Gravebound/actions/runs/29583478039) proved migration `0061` and reached the complete post-commit graph after every injected rollback; the remaining failure was test-only type decoding. The authenticated service should enter only the disposable real-QUIC route after the corrected hosted proof. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
