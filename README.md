...
Continue `GB-M03-04D` under the approved production reward RNG, opaque 16-byte UID, starter-Belt, and reward-RunBackpack placement contracts. Typed item/receipt/ledger migrations, canonical UID vectors, deterministic merge/overflow planning, and the replay-first exact-shape starter repository are complete; build the authoritative starter-plan service and new/existing-character invocation, obtain disposable-PostgreSQL replay/concurrency evidence, then implement the secret-backed reward transaction, durable pending/personal-ground placement, and atomic expiry against the completed `GB-M03-04B/04C` catalog. Finish `GB-M03-04A` by resolving the restore-point binding for post-entry XP/first-clear revocation, implementing its progression restore provider, and completing disposable-PostgreSQL evidence, full gates, ADR, and audit; exact content/simulation, durable replay-first awards, authoritative QUIC projection, and the accessible native HUD are complete. Finish `GB-M03-03B` with atomic disposable danger-entry capture and PostgreSQL closure evidence; its approved typed revision, return-arrival contract, authority-level payload-hash rejection, and replay-first safe coordinator are complete. Finish `GB-M03-05B`: its bounded reliable protocol, exact Oath schema, replay-first serializable repository, single character-life version, durable `oath_selected` outbox, and authoritative eligibility planner are complete; join inventory safety to the same transaction through `GB-M03-04D/04F`, then wire the reliable route, native permanent-life confirmation UI, disposable-PostgreSQL evidence, and full gates. Finish `GB-M03-05C`: exact live Long Vigil/Nailkeeper definitions and maximum-health inputs, approved cadence composition, deterministic trap lifecycle, real Grave Mark contact creation, trap-specific physical damage, normal-enemy Frostbind, boss immunity feedback, native trap visuals, and exhaustive Core Oath/Bargain/crossbow cap fixtures are complete; route persisted Oaths into the combat factory and add accessible audio/reduced-effects evidence. Begin the approved `SPEC-CONFLICT-009` death foundation when its item/location dependencies are ready. Pull the minimal `GB-M03-12` Ash wallet forward before `GB-M03-05D`. Keep the normal player route and affected Hall stations fail-closed until items, Oaths/Bargains, death, extraction, and Recall all pass.
