...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now owns the exact `pack.bell.01` lifecycle, reusable `DNG-005` authority, collision-free deterministic B1/B2/B3/B5 plans, complete immutable B1/B5 room owners, the exact nine-actor mixed B2 health/completion/reset owner and replay-identical sustained live-cycle trace, immutable Mire/Acolyte/Skull target/cast locks, approved exact Acolyte/Skull locomotion under [`SPEC-CONFLICT-017`](docs/spec-conflicts/SPEC-CONFLICT-017-m03-core-locomotion-phase.md), and shared hostile materialization with deterministic Choir Skull attack phase under [`SPEC-CONFLICT-018`](docs/spec-conflicts/SPEC-CONFLICT-018-m03-choir-skull-rotor-phase.md); B3 remains explicitly fail-closed pending its complete combat owner. Build the dedicated B3 Sepulcher Knight owner next, including its introduction, charge/contact, stop-ring/fan hostile execution, reset, and duration trace. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
