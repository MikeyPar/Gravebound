...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Under the canonical GDD, Content Production Spec, and Development Roadmap, commits `fb8f56a` through `6faf3bd` define, load, replay, reconnect, and process-restart the versioned canonical death/destruction/Memorial/Echo terminal signature. Commits `9ac2c39`, `e20a323`, and `d302b1c` then prove fail-closed altered summary, Memorial, destruction, Echo, Bargain-cleanup, and death-outbox reconstruction and add migration `0052`, which makes character-life outbox authority append-only while permitting one publication transition. Hosted CI `29484105278` is green for PostgreSQL 17, strict lint/tests/content/schema/determinism checks, and the optimized Windows build. The active [`GB-M03-06E`](docs/tasks/GB-M03-06E.md) step is the missing/duplicate causal-row cardinality matrix followed by measured durable-commit-to-interactive/query latency, task/session/transaction/lock cleanup, stable memory, branch coverage, and the joint `06`/`02D`/`13` audits. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
