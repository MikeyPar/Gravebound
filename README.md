...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282). Commits through `c9f8705` retain canonical damage facts and extend the [lossless terminal event stream](docs/evidence/GB-M03-03G-terminal-frame-feed-evidence.md) across Bell, every B0-B6 advance, fresh B3/B4 durable results, and the Caldus exit. Commit `2e02b94` adds the [opaque danger-entry authority](docs/evidence/GB-M03-03G-danger-entry-authority-evidence.md). Commit `0991cd6` adds the [mandatory terminal-owner lifecycle](docs/evidence/GB-M03-03G-terminal-owner-lifecycle-evidence.md): production can no longer spawn an ownerless danger driver, and owner start/join now brackets driver/tick lifetime across `LinkLost`, reconnect, unbind, and shutdown. Next implement the real owner with trace, independent clocks, deeds, custody, entity identities, per-tick providers, and all five terminal producers; normal admission remains fail closed until that factory exists.
