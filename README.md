...
> **Project status:** M01 and M02 are closed under their recorded gates. M03 has closed identity, parent PostgreSQL persistence, exact world-flow content, atomic dormant transfers, Hall/private-microrealm simulation and native evidence, progression, the complete Core 18-item/equipment/CharacterSafe/Vault lifecycle, the first Oath/Bargain package, the minimal Ash wallet, and atomic death/destruction/Memorial/Echo persistence with hosted adverse, outage, response-loss, restart, soak, and native evidence. Parents `GB-M03-02`, `GB-M03-04`, and `GB-M03-06` are complete. `GB-M03-08` now has hosted-green protocol `1.15`, terminal custody `0055`, atomic extraction graph `0056`, deterministic placement, a replay-first serializable writer, pre-arbitration plan sealing, shared-terminal execution, strict restart recovery, and a no-second-write Hall projection. Authenticated extraction dispatch, Emergency Recall, ResolutionHold recovery, successor recovery, complete-route admission, telemetry, support, platform evidence, and final private-loop/cohort gates remain disabled.
...
Parents [`GB-M03-02`](docs/milestones/GB-M03-02-audit.md), [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and [`GB-M03-06`](docs/milestones/GB-M03-06-audit.md), plus atomic Echo package [`GB-M03-13`](docs/milestones/GB-M03-13-audit.md), are complete. Hosted CI [`29506273492`](https://github.com/MikeyPar/Gravebound/actions/runs/29506273492) is the final death/Echo acceptance. [`GB-M03-08`](docs/tasks/GB-M03-08.md) now preserves old protocol bytes while appending extraction/Recall kinds `19`/`20`; adds exact Overflow/ResolutionHold custody in `0055`; supplies atomic graph `0056`; and has a hosted-green replay/restart/adverse PostgreSQL writer in [`29517763161`](https://github.com/MikeyPar/Gravebound/actions/runs/29517763161). Main additionally seals the exact placement plan before arbitration, executes only the shared terminal winner, recovers the committed graph after process loss, and projects Hall without a second version advance. Validate those newer layers in the active CI runs, then add authenticated extraction result mapping/dispatch and the real-QUIC matrix. After that, complete Emergency Recall, `GB-M03-07` successor recovery, and parent `GB-M03-03`; keep normal route admission and Core promotion fail closed.
