...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now owns the exact `pack.bell.01` lifecycle, reusable `DNG-005` authority, collision-free deterministic B1/B2/B3/B5 plans, complete immutable B1/B5 owners, the exact nine-actor mixed B2 owner, the complete fixed B3 Sepulcher Knight owner, and the complete disabled-branch Choir Abbot fixture. Both minibosses have shared hostile execution, exact introductions/reset/quiet boundaries, typed reward/XP handoffs, deterministic phase/gap rules under [`SPEC-CONFLICT-019`](docs/spec-conflicts/SPEC-CONFLICT-019-m03-sepulcher-knight-charge-and-gap.md) and [`SPEC-CONFLICT-020`](docs/spec-conflicts/SPEC-CONFLICT-020-m03-choir-abbot-phase-and-gap.md), and replay-identical 35-second traces. Complete the remaining Mire Leech full-combat fixture next, then finish fixed-room/native evidence and the cumulative Core `6/2` acceptance gates. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
