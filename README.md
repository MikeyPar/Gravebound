...
`GB-M03-04A`, `GB-M03-04D`, `GB-M03-05B`, `GB-M03-05D`, `GB-M03-05E`, and `GB-M03-12` are complete with authorized PostgreSQL concurrency/restart evidence and completion audits. Continue [`GB-M03-05F`](docs/tasks/GB-M03-05F.md): its bounded Bell checkpoint DTO and lineage/content/version-bound PostgreSQL repository pass; next add the dormant character-life Core live aggregate with same-lineage reconnect and dangerous-room handoff continuity, then the 30-second resume scheduler, post-commit safe-transfer reset, and reusable death/retirement cleanup seam. Finish `GB-M03-03B` with the remaining composite danger-entry providers and PostgreSQL closure evidence. Close `GB-M03-05C` after the remaining inspected Nailkeeper/Frostbind audiovisual evidence; its database restart evidence passes. Continue `GB-M03-04E`/`04F` and the approved `SPEC-CONFLICT-009` death foundation as their item/location dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, the normal player route, and affected Hall stations fail closed until their owning gates pass.
