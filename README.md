...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Atomic death/Memorial/Echo has real-QUIC/PostgreSQL restart, replay, adverse, latency, soak, reachable-branch, and distinct-account concurrency evidence. Hosted run `29497431183` passes cumulative quality, Windows release, PostgreSQL, six-branch, and concurrency source gates. The server-owned `06C` planner now converts raw custody into exact Equipment, Belt, stacked RunBackpack, PersonalGround, and run-material destruction without caller-authored ordinals, post-versions, or ledger IDs; [`SPEC-CONFLICT-028`](docs/spec-conflicts/SPEC-CONFLICT-028-m03-runbackpack-unit-destruction-order.md) records the stacked-unit tie-break. Death closure remains open for Safe Equipment/Belt corruption rejection, non-vacuous CharacterSafe/Vault preservation, immutable-row and typed post-death command proof, and optimized native frame capture. The first native runner attempt reached software Vulkan and then failed on a missing `libxkbcommon-x11` runtime dependency. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
