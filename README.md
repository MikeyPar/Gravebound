...
Commits through `c2d18ba` complete the ordinary native Core route from Bell transfer through extraction or lethal death and successor recovery. A lethal terminal frame now waits on the durable death read barrier, opens the production death-summary UI, retains exact successor mutation authority, retires the exact completed danger task after the stored successor, refreshes the authoritative roster, and uses ordinary Character Select -> Hall for confirmation two. Migration `0065` also corrects Black/Unique Echo power and removes unauthoritative Core appearance/theme IDs. This follows the canonical GDD death/recovery rules, the Content Production Spec's Core successor and `CONT-ECHO-001` contracts, and the Development Roadmap `GB-M03-03`/`06`/`07`/`13` gates. The Current Next Step is to expose the read-only Memorial Wall from its authoritative Hall interaction, then finish telemetry, support lookup, hosting/platform packaging, the newest tester build, and the user-deferred full audit/external gates.
