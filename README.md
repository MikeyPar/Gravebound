...
The three-authority parent audit accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). The normal-route prerequisites through protocol `1.18`, exact content, the capacity-one actor, schema-62 generation authority, restart-stable identity, Bell mailbox ownership, and bounded client reordering are green at exact source `a893acf` under hosted CI [`29624519851`](https://github.com/MikeyPar/Gravebound/actions/runs/29624519851), including Linux quality/content/schema gates, mandatory PostgreSQL proof, optimized Windows construction, and optimized native evidence. Commit `71005d1` locally completes the shared per-transport writer for request responses and Recall pushes, poisons uncertain/cancelled/reset delivery, retries generation-racing committed Recall on the winning transport, and removes raw Core sequence seams; strict workspace Clippy and the complete non-gated server/QUIC regression pass. Run cumulative hosted proof for `71005d1` next. Then add the terminal-first PostgreSQL bootstrap projection and live extraction intent, dedicated `BoundCorePrivateLifeServer`, and ordinary native route. Keep normal capability advertisement, Character Select `Play`, and Realm Gate interaction disabled until the complete runtime and evidence pass; keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed. The [Sir Caldus silhouette drafts](Concept%20Art/m03-sir-caldus-combat-silhouette-drafts/README.md) remain provenance-recorded art-direction sources only, not runtime sprites or route evidence.
