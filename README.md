...
The native route is compiled from the exact Core world records and localization. Fixed-point collision/navigation, server-owned interaction projection, camera bounds, standard/reduced-motion presentation, and the Dormant -> Waiting -> Active -> Cleared lifecycle remain deterministic. Three stable object IDs now resolve to hash-locked presentation art in the disposable showcase while their compiled geometry remains visible and authoritative; see the [visual manifest](docs/evidence/GB-M03-03G-microrealm-landmark-visual-manifest.md). The normal player route remains disabled until live movement, combat, rewards, pending inventory, and all terminal producers are composed through the persistent session.
