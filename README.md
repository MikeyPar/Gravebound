...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **70% complete**: identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete Core item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, and successful extraction/Emergency Recall/ResolutionHold are closed under hosted evidence. [`GB-M03-08`](docs/milestones/GB-M03-08-audit.md) passed at commit `2535e8c` on hosted CI [`29554811453`](https://github.com/MikeyPar/Gravebound/actions/runs/29554811453). Successor recovery has append-only protocol `1.17`, reliable kind `23`, `core_successor_v1`, pinned `1.16` compatibility, migration `0060`, ordinary-death preset/reservation authority green through the complete hosted matrix in [`29571991103`](https://github.com/MikeyPar/Gravebound/actions/runs/29571991103), and one transaction-local `04D` starter initializer seam green under strict persistence proof. The replay-first atomic successor-creation writer is next. Normal route admission, telemetry, support, hosting/platform evidence, and final private-loop/cohort gates remain disabled.
...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Hosted CI [`29571991103`](https://github.com/MikeyPar/Gravebound/actions/runs/29571991103) is green for the corrected legal pre-existing-roster fixture, every PostgreSQL and death/Echo source check, optimized native evidence capture, strict Linux validation, and optimized Windows construction. The transaction-local `StarterItemPlan` persistence seam is now locally green without duplicating starter validation or writes. Implement the replay-first serializable successor-creation repository next. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
