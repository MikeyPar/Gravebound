...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice now has the strict durable-death/Echo contract (`cb7de1d`) and forward schema `0037` complete-graph closure (`be561ee`) in addition to the V3 danger-entry/crash-restore foundation. `77ee315` corrected the hosted life-metrics fixture to use the schema-authoritative initializer. Require exact-head hosted PostgreSQL application of schema `0037`, then finish the serializable death/Echo repository, normalized graph fixtures, protocol `1.13` kind `18`, authenticated dispatch, real-QUIC restart/replay proof, and the joint audits. Keep all player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal-route behavior fail closed.
