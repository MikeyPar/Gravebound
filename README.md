...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Under `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`, commits `fb8f56a` through `e16424e` define, replay, restart, adversely validate, and enforce exact causal-outbox cardinality for the versioned death/destruction/Memorial/Echo terminal signature. Hosted CI `29488569454` accepted ten authenticated real-QUIC/PostgreSQL journeys with a 12.868 ms maximum durable-commit-to-ready `DeathViewClientModel`, exact replay, stable canonical signatures, and zero measured residue; [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) separately proves the optimized native presentation. Dedicated workflow `29489909161` accepted the checked-in [`GB-M03-06E` 30-minute soak](docs/performance/GB-M03-06E-death-memory-soak.json): 8,509 authenticated journeys, 34,036 reads, 425 reconnects/replays/signature checks, stable supplemental combined-process memory, and zero database/server residue. Commits `2d61ab5`, `e32cf1f`, and `32e5f6f` close provenance authority and readiness through migration `0054`. Commit `0d9156e` adds the six reachable eligibility/account-availability branches over production trace, terminal writer, authenticated real QUIC, PostgreSQL, exact replay, reconnect, stored signatures, and measured cleanup. Hosted runs `29492700515` and `29493375190` exposed one exact production defect after every preceding PostgreSQL suite passed: the existing-Echo tail query widened its `SMALLINT` fallback to `INT4`; hosted run `29495477261` now passes the repaired exact `SMALLINT` contract. The authenticated-fixture native frame probe is implemented over the real `DeathFinal` handoff, production focus/input path, render readiness, and captured-frame callback; 155 client tests, strict Clippy, and an optimized Windows build pass locally. The only reachable missing concurrency case is also implemented: two distinct accounts commit eligible deaths simultaneously, each receives one isolated Available Echo, and both exact concurrent replays preserve signatures and row counts. The active [`GB-M03-06E`](docs/tasks/GB-M03-06E.md) step is to accept and archive the hosted branch, concurrency, and native standard/reduced-effects frame reports, then publish the joint `06`/`02D`/`13` audits. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
