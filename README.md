...
[`GB-M03-03E`](docs/tasks/GB-M03-03E.md) is active under the three-authority contract and approved `SPEC-CONFLICT-022`/`023`. Strict non-promotable Sir Caldus, four-pattern, B6 overlay, exact reward/XP, and receipt-gated Hall-exit content now compile with independent hashes, schemas, mutation tests, and CI validation. Begin renderer-independent DNG-006 staging, participant lock, scaled-health allocation, introduction, late-entry rejection, and reset authority while combat and later layers remain fail closed. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
