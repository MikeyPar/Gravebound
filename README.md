...
[`GB-M03-04F`](docs/milestones/GB-M03-04F-audit.md) is complete under its three-authority audit: schema-28 CharacterSafe/Vault custody, the exact 8/160/8 planner, normalized schema-29/30 serializable transfers, bounded dormant coordination, and all-or-nothing danger-entry auto-deposit pass the full hosted PostgreSQL/release/quality matrix. [`GB-M03-04G`](docs/tasks/GB-M03-04G.md) is active: protocol 1.12 appends bounded safe-inventory kind 17, and the authenticated persistent dispatch returns accepted or state-free typed rejected mutation results while the normal Core runtime remains nonadvertising and disabled. Add the headless helper and disposable real-QUIC/PostgreSQL transfer/replay/reconnect/restart fixture, then close the complete `04A`-`04F` lifecycle under adverse, visual, performance, and parent-audit gates. Keep death/extraction/Recall, Overflow/ResolutionHold, salvage/crafting, Core promotion, Character Select `Play`, Realm Gate and Vault station admission, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
