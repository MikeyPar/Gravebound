...
> **Project status:** M01 and M02 are closed. The current GB-M03 player route is assembled in [optimized tester r32](docs/evidence/GB-M03-tester-r32-manifest.md), while **15 of 23 fixed formal closure units are proven (65%)**. The stable denominator is 14 Roadmap work packages plus nine explicit M03 exit outcomes; partial gates do not count. The open engineering work is final production-route/25-full-loop proof plus bounded operational export for the hosted-green schema-78 `TEL-003` facts. The private cohort, hosting/backup rehearsal, destination/privacy approval, and Steamworks evidence remain external gates. See the [formal gate audit](docs/milestones/GB-M03-gate-audit.md) and [support completion audit](docs/milestones/GB-M03-10-audit.md).
...
Source `525b423` is packaged as [optimized tester r32](docs/evidence/GB-M03-tester-r32-manifest.md) with both executables, accessibility controls, protocol 1.25, and migrations through schema 78. Client/server release construction, strict content validation, CLI checks, the 12-second Local Lab launch, all six standalone M03 presentation modes, single-root ZIP inspection, and atomic stale-`dist` cleanup passed. Hosted run [`30028182217`](https://github.com/MikeyPar/Gravebound/actions/runs/30028182217) proved committed Realm Gate activation and live authored combat, then isolated defeated-snapshot targeting; commit `2e8d2f4` fixes that policy with a green authoritative clear regression. Run [`30047477525`](https://github.com/MikeyPar/Gravebound/actions/runs/30047477525) received the exact live entry route and snapshot but then exposed a harness liveness dependency fixed by commit `1ad117e`. Run [`30049060896`](https://github.com/MikeyPar/Gravebound/actions/runs/30049060896) then reached ordinary damage during the valid pre-first-checkpoint interval and exposed schema 43's contradictory foreign key from live trace ticks to the optional 30-second debug checkpoint. Forward-only schema 79 removes only that redundant key while preserving immutable danger-entry, retained-receipt, content, and runtime validation. Independently, commit `1f1d0fd` implements the remaining atomic `TEL-003` death facts; hosted run [`30048509525`](https://github.com/MikeyPar/Gravebound/actions/runs/30048509525) applies schema 78 and passes legacy upgrade/restart/replay plus every fresh PostgreSQL/redaction assertion. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is focused hosted schema-79/pre-checkpoint proof followed by the complete extraction/death/successor journey, then 25 complete ordinary loops, current aggregate timing, optimized Realm Gate capture, and bounded operational telemetry proof. Private-cohort comprehension, backup/restore, hosting, destination/privacy approval, and Steamworks evidence remain required. Formal progress remains **15/23 (65%)** until a whole package or exit outcome closes.
