...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282), including Linux quality gates, PostgreSQL, Windows release, and optimized native death frames. Commits `06ef41f`, `bb67be7`, and `e9a7e16` retain canonical player-damage tick/order/source/target/health/origin authority. Commit `68830ad` adds the reviewed [lossless terminal-frame feed](docs/evidence/GB-M03-03G-terminal-frame-feed-evidence.md): every bound frame is acknowledged before tick/presentation publication, lethal ownership requires the exact stored receipt, and stalled shutdown is explicit rather than deadlocked. Next total-order the Bell/room/reward/exit control transitions between frames, bind the exact danger-entry restore authority, and replace the disabled session's terminal-ownerless path with the live trace, clocks, deeds, custody, and five-producer terminal owner before normal admission opens.
