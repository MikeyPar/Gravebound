...
Commits `3c05a36`, `cdeceda`, `8efcec0`, `b61cbce`, `4eba061`, and `c433cbd` now carry B5 authority into a route-bound B6 owner: exact compiled arena, player/combat/projectile identity, inherited 30 Hz tick, five-second countdown, immutable participant lock, resumable 75-tick introduction, same-tick Phase 1 encounter, legal early-phase defeat projection, and stale-route rollback are locally green. Commit `7669e41` supplies the separately verified unregistered transition pack. Next add the typed `0.70` Caldus physical-body collision path for walking, Slipstep, and charge de-overlap while retaining the separate `0.62` damage hurtbox; then run the complete deterministic fight through defeat before composing committed boss reward/pending-inventory placement and the stable exit. Bound normal-server construction, hosted B3 adverse/restart proof, and normal admission remain disabled.
