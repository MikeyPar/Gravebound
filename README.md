...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). The replay-first serializable writer now creates identity, progression, world, life, Oath/Bargain, exact `04D` starter state, selection, immutable result/receipt/audit/outbox, and reservation consumption in one transaction. Push it through the mandatory hosted `postgres_successor` gate; after that proof is green, add the authenticated service and disposable real-QUIC route. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
