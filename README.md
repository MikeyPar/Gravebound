...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Hosted run [`29585161647`](https://github.com/MikeyPar/Gravebound/actions/runs/29585161647) closes the atomic writer/service database gate. The persistent real-QUIC response-loss, reconnect, byte-identical replay, pool/authority restart, altered-payload conflict, and exact selected four-item graph journey is implemented and locally compile/lint clean. Run it in the mandatory hosted PostgreSQL gate; after that passes, add the separate full child-process restart plus feature-absent, malformed/oversized, and zero-residue adverse matrix. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
