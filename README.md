...
Commit `3167db9` publishes the accepted optimized 24-frame Resolution Hold matrix with exact dimensions, build/content identity, independent decode review, and per-file SHA-256 in the [visual manifest](docs/evidence/GB-M03-08-hold-visual-manifest.md). Production Recall now has monotonic transport generations plus a bounded per-character actor directory: stale authorities fail closed, the actor tick is sampled after dequeue, and planned shutdown closes inboxes before joining connection workers with zero actor/transport residue. Its real-QUIC delivery task consumes committed completion publications through the same sequence authority as request responses and reports delivery/drain residue explicitly. A disposable hosted matrix now covers tick `89`/`90`, reconnect push and restart recovery, duplicate handoff/stale detach, real lethal precedence, and zero Recall/transport/actor/PostgreSQL residue. Next, obtain hosted-green evidence and audit `GB-M03-08`. Then complete `GB-M03-07` successor recovery and parent `GB-M03-03`; normal route admission and Core promotion remain fail closed.
