...
[`GB-M03-03E`](docs/tasks/GB-M03-03E.md) is active under the three-authority contract and approved `SPEC-CONFLICT-022`–`024`. Strict content, DNG-006 lock/reset, exact combat, 180-second replay, stable identities, and shared SOC-010 eligibility pass their current gates. Schema 25 and the server victory coordinator now compose deterministic Core item rewards, durable 450-XP/first-clear progression, and a stable exit that cannot commit until every eligible owner has both exact non-revoked terminals; partial completion is replay-safe and contains no GB-M03-08 inventory conversion. Add disposable PostgreSQL fresh/replay/mismatch/partial-terminal evidence next, then expose the committed exit; extraction and later layers remain fail closed. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
