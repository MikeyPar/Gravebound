...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **76% complete**: identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete Core item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, and successful extraction/Emergency Recall/ResolutionHold are closed under hosted evidence. [`GB-M03-08`](docs/milestones/GB-M03-08-audit.md) passed at commit `2535e8c` on hosted CI [`29554811453`](https://github.com/MikeyPar/Gravebound/actions/runs/29554811453). Successor recovery has append-only protocol `1.17`, reliable kind `23`, `core_successor_v1`, pinned `1.16` compatibility, migrations `0060`/`0061`, universal death presets/reservations, the shared transaction-local exact `04D` starter seam, a replay-first atomic writer, and authenticated fail-closed service. Hosted run [`29589599121`](https://github.com/MikeyPar/Gravebound/actions/runs/29589599121) proves real-QUIC response loss, reconnect, PostgreSQL-pool reconstruction, byte-identical stored replay, altered-payload conflict, one exact successor/starter graph, and bounded resource cleanup. Corrected cumulative run [`29595244036`](https://github.com/MikeyPar/Gravebound/actions/runs/29595244036) is fully green for the adverse feature-absent/malformed/oversized ingress matrix and the same exact replay from a fresh OS integration-test process, alongside strict Linux quality, all migrations/transactions, optimized Windows release, and native death evidence. The shipped normal runtime remains disabled. A renderer-independent native recovery authority is locally green for negotiated/durable-summary gating, exact create-frame retry, all typed rejections, one preselected successor projection, and Play as confirmation two behind authoritative Hall readiness. Its schema-backed, transitive presentation target pins the exact identity, death, item, Hall, silhouette, and closed-copy dependencies. The native death surface exposes `Create Successor` only through a matching opaque durable-terminal proof; a reusable post-create plugin renders every authoritative recovery phase; and the evidence-only coordinator composes the real widgets through Create, preselected Character Select, Play, Hall transfer, and scene readiness without changing normal startup. The optimized 32-frame resolution/effects/scale matrix passes. A hashed 25-journey real-QUIC/PostgreSQL harness is locally strict-green and awaits hosted execution; it covers unique durable deaths, both confirmations, exact Hall/danger readiness, fresh starter identities, timings, duplicate checks, and zero operational residue. Normal route admission, telemetry, support, hosting/platform evidence, and final private-loop/cohort gates remain disabled.
...
The evidence-only native coordinator joins the durable terminal proof, exact stored successor result, preselected Character Select, accepted Hall transfer, and matching scene readiness without enabling the shipped normal route. The [32-frame optimized visual matrix](docs/evidence/GB-M03-07-successor-visual-manifest.md) covers every recovery phase, both certified resolutions and effects modes, and 150% Character Select scale. Normal Play and production Realm Gate admission remain fail-closed until the journey/timing and parent integration gates pass.
...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Hosted run [`29595244036`](https://github.com/MikeyPar/Gravebound/actions/runs/29595244036) closes the cumulative response-loss/reconnect/pool/OS-process replay, adverse ingress, and bounded-residue transport gate. The optimized [32-frame native matrix](docs/evidence/GB-M03-07-successor-visual-manifest.md) passes every recovery state at both certified resolutions/effects modes plus 150% scale. Run the new hashed 25-journey successor-control/danger harness in hosted PostgreSQL, publish its immutable timing/identity/residue report, then complete the three-authority audit. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
