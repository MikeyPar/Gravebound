...
Commits `3c05a36` and `cdeceda` remove the B6 tick-rewind and ownership gaps: the fixed-dungeon owner now consumes B5 only from authoritative B6 `BossStaging` and transfers the exact route lease, content revision, character envelope, player allocation, projectile allocator, and inherited danger tick. Commit `7669e41` adds the separately hash-verified, unregistered staging/countdown/introduction review pack shown above. Next construct `CorePrivateCaldusRuntime` from that handoff and drive exact loading, five-second countdown, participant lock, 2.5-second introduction, and combat phase projection before composing committed boss reward/pending-inventory placement and the stable exit. Bound normal-server construction, hosted B3 adverse/restart proof, and normal admission remain disabled.
