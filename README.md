...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **96% complete**. Identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, extraction/Emergency Recall/ResolutionHold, and successor recovery are closed under hosted evidence. The active private-route integration now owns clocks, deeds, damage evidence, death planning, atomic death/Echo execution, and one coherent current-danger snapshot; lethal health, explicit Recall, and LinkLost disconnect recovery now enter the same route-bound terminal coordinator. Remaining work is composing extraction and verified-fault restoration through that owner, adding exit-ready terminal heartbeats, then ordinary dispatch, telemetry/support/hosting, and final hosted/private-cohort/platform gates.
...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282). Commits `8cd626e`, `50d858a`, `5d9dc33`, and `1ec0285` extend the [durable terminal-owner evidence](docs/evidence/GB-M03-03G-durable-terminal-owner-evidence.md): ordinary frames cannot advance before exact clock/trace durability; B3/Caldus controls cannot acknowledge before deed projection; one serializable PostgreSQL read supplies all eight Recall versions, extraction custody, clocks, pending counts, and active root; and lethal health, explicit Recall, and LinkLost disconnect recovery now arbitrate through the exact route-bound Recall actor before any frame acknowledgement. Next route the prepared Caldus extraction actor into this owner, continue terminal heartbeats in Caldus exit-ready state, and add the typed verified-fault restoration producer. Normal admission remains fail closed until all five producers share this coordinator and focused restart/shutdown proof passes.
