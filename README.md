...
Commits through `acc721f` complete the ordinary native Core route from Bell transfer through extraction or lethal death, Memorial, and successor recovery. The production death surface now opens its exact stored Memorial snapshot, while Hall exposes the same bounded newest-first read-only Wall with pagination, selection, retry, and close behavior. This follows the canonical GDD death/recovery rules, the Content Production Spec's `CONT-HUB-002` Memorial contract, and the Development Roadmap `GB-M03-03`/`06`/`07` gates. The Current Next Step is to finish telemetry, authenticated support lookup, hosting/platform packaging, the newest tester build, and the user-deferred full audit/external gates.
