...
Close `GB-M03-04D`: typed item/reward/provenance migrations, exact starter reconciliation, secret-backed ChaCha8 planning, opaque unit UIDs, deterministic RunBackpack/personal-ground placement, pre-plan request reservation, epoch-rotation-safe replay, immutable ledger writes, and exact-tick atomic ground expiry are implemented; workspace tests, warnings-denied Clippy, ADR-030, and normal-route gating pass. Execute the compiled starter/reward/expiry restart and concurrency fixture against an authorized disposable PostgreSQL database, then write the completion audit. Close `GB-M03-04A`: exact content/simulation, durable replay-first awards, authoritative QUIC projection, accessible native HUD, exact danger restore binding, monotonic entry XP/level/health restoration, immutable receipt revocation, exact first-clear rollback, typed `revoked_by_crash_restore` replay, full workspace gates, and ADR-031 are complete. Execute the compiled progression concurrency/crash-restore and real-QUIC fixtures against an authorized disposable PostgreSQL database, then write the audit. Finish `GB-M03-03B` with atomic disposable danger-entry capture and PostgreSQL closure evidence; its approved typed revision, return-arrival contract, authority-level payload-hash rejection, and replay-first safe coordinator are complete. Close `GB-M03-05B`: its bounded reliable protocol, exact Oath schema, replay-first serializable repository, single character-life version, durable `oath_selected` outbox, authoritative eligibility planner, atomic inventory-safety join, persistent reliable route, restart fixture, exact localization, native permanent-life confirmation UI, and full workspace gates are complete. Execute the compiled repository and real-QUIC fixtures against an authorized disposable PostgreSQL database, then write the audit. Close `GB-M03-05C`: exact live definitions, cadence composition, trap lifecycle, Grave Mark contact creation, trap damage/Frostbind, boss immunity, atomic persisted-loadout projection, fail-closed combat factory, restart fixture, native visuals, distinct audio, reduced-motion readability, exhaustive combination fixtures, ADR-033, and full workspace gates are complete. Execute the combined PostgreSQL fixture and inspect audiovisual evidence, then write the audit. Close `GB-M03-12`: the 99,999-cap account wallet, forward-only migrations, expected-version binding, typed exact source/sink contracts, replay-first serializable earn/spend authority, immutable accepted-mutation ledger, restart/concurrency fixture, ADR-034, mandatory local/CI PostgreSQL wiring, normal-route gating, and full workspace gates are complete. Execute the compiled Ash fixture against an authorized disposable PostgreSQL database, then write the completion audit and begin `GB-M03-05D`. Begin the approved `SPEC-CONFLICT-009` death foundation when its item/location dependencies are ready. Keep the normal player route and affected Hall stations fail-closed until items, Oaths/Bargains, death, extraction, and Recall all pass.
