...
`GB-M03-04A`, `GB-M03-04D`, `GB-M03-05B`, and `GB-M03-12` are complete with authorized PostgreSQL concurrency/restart evidence and completion audits. Finish `GB-M03-05D`: its exact `CONT-014` milestone transaction, deterministic offer, replay-first select/refuse authority, protocol/runtime route, and accessible native shrine are implemented. Add mandatory PostgreSQL concurrency/restart evidence, prove the shrine over real QUIC, inspect the native surface, run the full closure gates, and write the completion audit before advancing to `GB-M03-05E`. Finish `GB-M03-03B` with the remaining composite danger-entry providers and PostgreSQL closure evidence. Close `GB-M03-05C` only after inspecting Nailkeeper arm/armed/trigger, Frostbind immunity audio, and reduced-motion presentation; its database restart evidence now passes. Continue `GB-M03-04E`/`04F` and the approved `SPEC-CONFLICT-009` death foundation as their item/location dependencies become ready. Keep the normal player route and affected Hall stations fail-closed until items, Oaths/Bargains, death, extraction, and Recall all pass.
