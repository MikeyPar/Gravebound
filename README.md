...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **94% complete**. Identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, extraction/Emergency Recall/ResolutionHold, and successor recovery are closed under hosted evidence. The active private-route integration now resolves Sir Caldus rewards, activates production extraction, survives `LinkLost`/reconnect, captures lethal tick context and immutable entity provenance, and can seal a coherent PostgreSQL-backed durable-death plan. Remaining work is connecting that plan to the production terminal owner and all five producers, danger activation and ordinary dispatch, the dedicated bound normal route, telemetry/support/hosting packages, and final hosted/private-cohort/platform gates.
...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282). Commits through `e869f01` retain the lossless route stream, opaque danger authority, mandatory owner lifecycle, durable trace, and same-boundary terminal context. Commits `234fa2c`, `ff22530`, `3238a0a`, and `8d71a04` add [commit-time Echo revalidation and the sealed death-context planner](docs/evidence/GB-M03-03G-death-context-planner-evidence.md): one coherent PostgreSQL snapshot reconstructs clocks, deeds, custody, versions, route room, and Echo state, while the lethal trace carries its own immutable entity map. Next install this planner in the production terminal-owner factory, persist clocks and deeds continuously, wire the server-only Recall phase, and hold lethal acknowledgement until the exact durable receipt commits. Then connect extraction, Recall, disconnect recovery, and server-fault restoration through the same five-producer coordinator; normal admission remains fail closed until that owner graph passes three-scene restart and shutdown proof.
