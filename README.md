...
Commits `3f749dd`, `e7905ea`, and `e62587b` now reserve the exact `BossExitReady` route before the final custody read, preserve storage versions separately from the wire projection, construct/register the production extraction actor, bind it to the existing private-life writer, and prove the automatic post-acknowledgement order through activation. Fresh/replayed ownership prevents early, exact, or altered replay from aborting the winner; real-QUIC proof now covers active ownership, `LinkLost` survival, exact reconnect to the same microrealm and shared writer, transport-free retirement, route reopening, and no extraction resurrection on a later connection. Fail-closed cleanup, terminal-completion release, and unresolved-owner teardown gates are covered in the [activation evidence](docs/evidence/GB-M03-03G-caldus-extraction-activation-evidence.md). Next drive the full live worker from `CaldusRewardPending` through durable resolution/acknowledgement, including response loss and competing-terminal completion through the actual runtime. Then run hosted PostgreSQL/real-QUIC adverse and restart proof. Bound normal-server construction and normal admission remain disabled.
