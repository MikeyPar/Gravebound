...
[`GB-M03-03E`](docs/milestones/GB-M03-03E-audit.md) is complete under its three-authority audit. Strict Caldus content, the DNG-006 lock/reset, exact 180-second combat, personal reward terminality, a reward-gated stable exit, committed extraction, idempotent `HallDefault` transfer, crash/retry/rollback recovery, mandatory PostgreSQL coverage, and a 32-frame native standard/reduced-effects evidence matrix pass without importing `GB-M03-08` inventory conversion or opening normal ingress. Open the three-authority `GB-M03-03F` contract for loading/error/reconnect UX, real-QUIC journey and failure evidence, and final visual/performance closure. Continue the parent roadmap audit, `GB-M03-04E`-`04G`, and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
