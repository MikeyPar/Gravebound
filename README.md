...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md) slice now has schemas `0031`-`0033`: normalized death/summary/memorial/trace/Echo/material records, strict replay/ledger/ownership/history bindings, and a component-complete danger-entry restore V2. V2 atomically captures progression, exact Equipped/Belt identities, Oath/Bargains, and both life clocks; it transitions safe loadout items to `AtRiskEquipped` with deterministic ledgers and one combined inventory-version advance. Require exact-head hosted PostgreSQL proof next, then implement exact crash restoration and the strict death repository/protocol. Keep all player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal-route behavior fail closed.
