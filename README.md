...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **95% complete**. Identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, extraction/Emergency Recall/ResolutionHold, and successor recovery are closed under hosted evidence. The active private-route integration now owns clocks, deeds, damage evidence, death planning, atomic death/Echo execution, and durable lethal acknowledgement; the Recall actor also applies its canonical movement/action constraints and trace phase at the exact simulation boundary. Remaining work is composing Recall, extraction, disconnect, and verified-fault candidates through that same owner, adding exit-ready terminal heartbeats and the coherent current-terminal snapshot, then ordinary dispatch, telemetry/support/hosting, and final hosted/private-cohort/platform gates.
...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282). Commits `8cd626e` and `50d858a` add the [durable terminal owner and same-boundary Recall projection](docs/evidence/GB-M03-03G-durable-terminal-owner-evidence.md): ordinary frames cannot advance before exact clock/trace durability; B3/Caldus controls cannot acknowledge before deed projection; lethal damage cannot acknowledge before the five-way barrier selects the sealed death and PostgreSQL returns its atomic death/Memorial/Echo receipt; and Recall applies 75% movement, action suppression, and trace state at the retained-input boundary. Next add one coherent current-terminal snapshot, feed explicit/LinkLost Recall and extraction candidates into this owner, continue terminal heartbeats in Caldus exit-ready state, and add the verified-fault producer. Normal admission remains fail closed until those producers share one coordinator and focused restart/shutdown proof passes.
