...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schemas `0043`–`0044` add the normalized live `06B` evidence foundation and complete mutation authority: append-only clock and reward-qualified deed receipts plus a bounded ten-second trace graph that remains separate from the Bell checkpoint payload and must be removed by terminal death. The internal terminal coordinator requires all five server producers to evaluate a tick before sealing and preserves death's absolute same-tick precedence; the death builder, execution adapter, and committed-graph recovery projection remain the only lethal write/restart path. Next implement the live-evidence transactional repositories and server ingestion, wire the coordinator to execution/recovery, and prove committed death through hosted `PostgreSQL` and real QUIC with response loss/restart. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
