...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **92% complete**. Identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, extraction/Emergency Recall/ResolutionHold, and successor recovery are closed under hosted evidence. The active private-route integration now automatically resolves Sir Caldus rewards, reloads one coherent post-reward danger snapshot, publishes bounded pending inventory before `BossExitReady` over protocol 1.19, and replays both across reconnect. Remaining work is exact production extraction actor construction/registration and session binding, bound normal admission, telemetry/support/hosting packages, and final hosted/private-cohort/platform gates.
...
Commits `3f749dd`, `e7905ea`, `e62587b`, and `6d4fed6` now cover the complete local `CaldusRewardPending` worker through durable resolution, exact retry, acknowledgement, reservation-before-custody, production extraction activation, retained publication, dropped unacknowledged QUIC responses, newer-generation replay, `LinkLost` survival, exact reconnect, terminal-completion release, transport-free retirement, route reopening, and no extraction resurrection. Fresh/replayed ownership prevents early, exact, or altered replay from aborting the winner, and every local check is captured in the [activation evidence](docs/evidence/GB-M03-03G-caldus-extraction-activation-evidence.md). Hosted CI proved the repaired B3 cases green, then exposed stale Caldus test authority; commits `58e417e` and `6d1c0eb` restore the canonical progression/inactivity boundaries, isolate the Caldus IDs, and seed the required active lineage without weakening production constraints. Next obtain a green hosted PostgreSQL/real-QUIC run for this source, then close the remaining live death/terminal/tick-source composition prerequisites before any normal admission gate opens.
