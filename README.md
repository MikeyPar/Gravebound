...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now owns the exact `pack.bell.01` lifecycle, reusable `DNG-005` authority, deterministic B1/B2/B3/B5 plans, complete immutable B1/B5 room owners, and shared exact health/death/reward/spawn-invulnerability authority for all five Core-authored actors; B2/B3 remain explicitly fail-closed pending their complete authored combat owners. Resolve [`SPEC-CONFLICT-017`](docs/spec-conflicts/SPEC-CONFLICT-017-m03-core-locomotion-phase.md) while continuing B2's movement-independent combat seams, then bind locomotion and implement the dedicated B3 Knight owner. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
