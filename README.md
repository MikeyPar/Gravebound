...
Source `eb0c6b4` is packaged as [optimized tester r36](docs/evidence/GB-M03-tester-r36-manifest.md). Its primary launcher carries pinned PostgreSQL 17.10, starts it on loopback, preserves wipeable data under `.runtime`, and reaches the real schema-79 server without Docker. Commit `eb0c6b4` also corrects the named B1 persistence defect by transactionally advancing the open danger lineage's current content ID during the exact same-lineage Bell scene transfer; focused PostgreSQL restart/replay proof passes. Hosted run [`30069213403`](https://github.com/MikeyPar/Gravebound/actions/runs/30069213403) passed its schema-79/pre-checkpoint gate at that exact source and is executing the assembled real-QUIC route. The exact-25 evidence compiler exists at commit `886387f`; it does not claim 25 completed private loops yet. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is hosted green single-route completion, then 25 complete ordinary loops, current aggregate timing, optimized Realm Gate capture, and bounded operational telemetry proof. Private-cohort comprehension, backup/restore, hosting, destination/privacy approval, and Steamworks evidence remain required. Formal progress remains **15/23 (65%)** until a whole package or exit outcome closes.
