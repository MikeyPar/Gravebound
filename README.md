...
Commits through `b252a3b` complete the [durable terminal-owner boundary](docs/evidence/GB-M03-03G-durable-terminal-owner-evidence.md). One serializable PostgreSQL read supplies every terminal version, custody row, clock, count, and active root; all five producers arbitrate before acknowledgement; pure runtime faults advance one typed terminal boundary and invoke the exact TECH-023 restore transaction; indeterminate durable-control failures remain restart-only; and a successful restoration retires stale danger authority and installs a fresh Hall actor before acknowledgement. The Current Next Step is ordinary authenticated dispatch and capability admission for the complete Character Select -> Hall -> private route -> terminal return loop, followed by the user-deferred cumulative audit. Normal admission remains fail closed while that dispatch composition is incomplete.
