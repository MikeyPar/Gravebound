...
Commit `3167db9` publishes the accepted optimized 24-frame Resolution Hold matrix with exact dimensions, build/content identity, independent decode review, and per-file SHA-256 in the [visual manifest](docs/evidence/GB-M03-08-hold-visual-manifest.md). The production Recall session lifecycle now owns monotonic transport generations, stale-detach protection, terminal-preserving late reconnect, and nonterminal planned shutdown. Next, bind that lifecycle to the bounded actor/outbox runtime and close the PostgreSQL/real-QUIC tick-`90`/lethal zero-residue matrix before the `GB-M03-08` audit. Then complete `GB-M03-07` successor recovery and parent `GB-M03-03`; normal route admission and Core promotion remain fail closed.
