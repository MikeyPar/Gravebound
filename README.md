...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Protocol `1.17`/kind `23` is green across 76 focused tests and the all-target workspace compile. Next add migration `0060`, then one serializable writer that persists the universal preset/reservation/result and reuses `StarterItemPlan` inside the successor transaction. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
