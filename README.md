...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now compiles all eight roster actors without constructing runtime entities: three immutable First Playable definitions plus five Core definitions with exact 30 Hz warnings, cycles, quiet windows, projectile lifetimes, and trace-checked scheduler maxima. Prove the authored COM-006 minimum-speed routes next, including rotating-arm tracking and close-spawn boundaries, then implement target selection, state transitions, reset, and encounter construction. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as their dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
