...
`GB-M03-04A`, `GB-M03-04D`, `GB-M03-05B`, and `GB-M03-12` are complete with authorized PostgreSQL concurrency/restart evidence and completion audits. Implement `GB-M03-05D` next: establish the shared Oath/Bargain life aggregate and integrate the exact `CONT-014` milestone slot/offer disposition into the progression transaction, then add replay-safe select/refuse protocol, persistent authority, and the accessible three-cell shrine UI. Finish `GB-M03-03B` with the remaining composite danger-entry providers and PostgreSQL closure evidence. Close `GB-M03-05C` only after inspecting Nailkeeper arm/armed/trigger, Frostbind immunity audio, and reduced-motion presentation; its database restart evidence now passes. Continue `GB-M03-04E`/`04F` and begin the approved `SPEC-CONFLICT-009` death foundation as their item/location dependencies become ready. Keep the normal player route and affected Hall stations fail-closed until items, Oaths/Bargains, death, extraction, and Recall all pass.
