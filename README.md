...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now owns the exact `pack.bell.01` lifecycle, reusable `DNG-005` authority, collision-free deterministic B1/B2/B3/B5 plans, complete immutable B1/B5 owners, the exact nine-actor mixed B2 owner and replay-identical sustained trace, and the complete fixed B3 Sepulcher Knight owner. B3 includes approved deterministic charge/contact and target-opposite gap rules under [`SPEC-CONFLICT-019`](docs/spec-conflicts/SPEC-CONFLICT-019-m03-sepulcher-knight-charge-and-gap.md), shared stop-ring/fan hostile execution, exact introduction/reset/quiet boundaries, typed miniboss reward/XP handoff, full-route retry identities, and a replay-identical 35-second trace. Build the complete Choir Abbot authored fixture next, then finish fixed-room/native evidence and the cumulative Core `6/2` acceptance gates. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
