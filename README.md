...
Source `503bb1a` is packaged as optimized tester r22 with the assembled native route, full-route accessibility controls, and forward schema-69 item authority. Client/server release construction, strict content validation, CLI checks, a 12-second Local Lab launch, and all seven standalone presentation modes passed; publication then atomically removed every stale `dist` artifact. Native mutation IDs remain nonzero UUIDv7 values across restart, and open danger clocks remain exact while closed terminal roots fail closed. `GB-M03-10` is complete under hosted PostgreSQL least-privilege proof. Schema 70 now has optional production-root logical-session wiring: accepted client platform, explicit environment/region attribution, durable-head restart recovery, generation-safe reconnect, clean client exit, and server shutdown endings remain bounded and cannot block gameplay. Its hosted first-combat fixture now creates required lineage/restore roots atomically and awaits rerun. The real-QUIC production-root journey reaches controllable Hall, traverses the authored Hall, opens the Realm Gate, commits the portal mutation, and reaches matching controllable Core-microrealm authority. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is to prove schema-70 session/onboarding/first-combat/end facts around that production-root journey; extend it through B0-B6, Caldus, extraction, death/successor, and 25 loops; then add crash collection, bind loot to its origin session, and wire the disabled exporter worker. Current optimized Realm Gate capture, the private cohort, comprehension metrics, backup/restore, hosting, and Steamworks proof remain required gates.
