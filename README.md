...
The three-authority parent audit accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). The append-only protocol `1.18`/native projection is green at exact source `342889b` under hosted CI [`29619764945`](https://github.com/MikeyPar/Gravebound/actions/runs/29619764945). Exact `CorePrivateLifeContent`, the capacity-one route-phase actor, and PostgreSQL migration `0062` generation allocator are green at exact source `c768a62` under hosted CI [`29622689323`](https://github.com/MikeyPar/Gravebound/actions/runs/29622689323), including Linux quality/content/schema gates, mandatory PostgreSQL restart/concurrency proof, optimized Windows construction, and optimized native evidence. Local commits `23fe3ae`, `b02f0f6`, and `d5fd282` add restart-stable character IDs, mailbox-serialized Bell transitions, and bounded contiguous cross-stream client delivery; run their cumulative hosted proof next. Then add the common per-transport writer, terminal-first bootstrap/live extraction intent, dedicated `BoundCorePrivateLifeServer`, and ordinary native route. Keep normal capability advertisement, Character Select `Play`, and Realm Gate interaction disabled until the complete runtime and evidence pass; keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed. The new [Sir Caldus silhouette drafts](Concept%20Art/m03-sir-caldus-combat-silhouette-drafts/README.md) are provenance-recorded art-direction sources only, not runtime sprites or route evidence.
