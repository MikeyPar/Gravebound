...
Source `0afc90e` remains packaged as [optimized tester r33](docs/evidence/GB-M03-tester-r33-manifest.md), but its primary launcher requires Docker and therefore does not meet the self-contained tester boundary. Hosted run [`30053995319`](https://github.com/MikeyPar/Gravebound/actions/runs/30053995319) passed schema-79 pre-checkpoint proof and the exact Bell B0 snapshot, then proved the production terminal owner dropped B1's first authoritative frame at route version 9/tick 1243; the public route correctly remained `RoomSpawnWarning` instead of inventing `RoomActive`. Commit `0698737` publishes the complete owner error immediately, and focused run [`30066885083`](https://github.com/MikeyPar/Gravebound/actions/runs/30066885083) is resolving the exact failing substep. In parallel, the r34 release correction packages pinned PostgreSQL 17.10 on loopback, preserves wipeable data under `.runtime`, and smoke-checks real server readiness without Docker before atomically replacing r33. The exact-25 evidence compiler exists at commit `886387f`; it does not claim 25 completed private loops yet. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is the smallest evidence-backed terminal-owner fix, a hosted green single route, the self-contained optimized r34 package, then 25 complete ordinary loops, current aggregate timing, optimized Realm Gate capture, and bounded operational telemetry proof. Private-cohort comprehension, backup/restore, hosting, destination/privacy approval, and Steamworks evidence remain required. Formal progress remains **15/23 (65%)** until a whole package or exit outcome closes.
