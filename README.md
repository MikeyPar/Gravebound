...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282). Commits through `68830ad` retain canonical damage facts and acknowledge every bound frame before publication. Commit `c9f8705` extends the same [lossless terminal event stream](docs/evidence/GB-M03-03G-terminal-frame-feed-evidence.md) across Bell, every B0-B6 advance, fresh B3/B4 durable results, and the Caldus exit; controls inherit simulation time and cannot be mistaken for the independent life clocks. Next bind the exact danger-entry restore authority and replace the disabled session's terminal-ownerless path with the live trace, clocks, deeds, custody, entity identities, per-tick providers, and five-producer terminal owner before normal admission opens.
