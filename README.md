...
The three-authority parent audit accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). The normal-route prerequisites through the single per-transport writer are green at exact source `2d59fa3` under hosted CI [`29625864479`](https://github.com/MikeyPar/Gravebound/actions/runs/29625864479): Linux format/lint/tests/content/schema gates, mandatory PostgreSQL transactions, optimized Windows construction, and optimized native death frames all pass. The terminal-first PostgreSQL bootstrap and live extraction-intent slices are implemented locally, but independent audits correctly held them for exact successor/death authority, cross-character restart binding, Hall-arrival validation, a directory-backed `TerminalPending` permit, paired route/world content authority, durable changed-intent auditing, and authoritative issue-time validation. Finish those corrections and their PostgreSQL/restart tests next, then compose the dedicated `BoundCorePrivateLifeServer` and ordinary native route. Keep normal capability advertisement, Character Select `Play`, and Realm Gate interaction disabled until the complete runtime and evidence pass; keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed. The unregistered [Sir Caldus motion-strip review pack](assets/core/bosses/sir_caldus/review/v2/README.md) now has reviewed four-frame Shield Arc and Charge Lane strips, locked anchors, 192/96 px readability, provenance, and standard/reduced camera mocks. It remains outside the asset registry and runtime content hashes pending in-engine timing, attack-origin/hurtbox, anchor-drift, and optimized native review.
