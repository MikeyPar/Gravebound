...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Under the canonical GDD, Content Production Spec, and Development Roadmap, commit `fb8f56a` now defines the versioned canonical death/destruction/Memorial/Echo terminal signature and a serializable connection-scoped terminal recovery seam. Its eight focused tests, all 178 persistence unit tests, strict all-target/all-feature persistence Clippy, workspace compilation, and an independent P0-P2-clean review are green. The active [`GB-M03-06E`](docs/tasks/GB-M03-06E.md) step is to load that signature from one PostgreSQL snapshot and prove byte-identical values before response loss, after exact replay, after reconnect, and after a newly bound server process; crash, concurrency, performance, cleanup, and the joint `06`/`02D`/`13` audits follow. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
