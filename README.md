...
[`GB-M03-03D`](docs/milestones/GB-M03-03D-audit.md) closes the strict unpromoted Core `6/2` roster, all nine Bell templates, exact micro-realm pack, fixed B1/B2/B3/B5 combat owners, complete Sepulcher Knight/Choir Abbot/Mire fixtures, inspected standard/reduced-effects evidence at 1920×1080 and 1280×720, and cumulative local/hosted gates. Begin `GB-M03-03E` with a fresh three-authority implementation contract for production Sir Caldus, the boss participant lock, committed extraction exit, and authoritative Hall return. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
