...
[`GB-M03-03E`](docs/tasks/GB-M03-03E.md) is active under the three-authority contract and approved `SPEC-CONFLICT-022`–`024`. Strict Caldus/exit content, DNG-006 lock/reset, the exact scheduler, shared hostiles, fixed-point body/contact/return, real health combat, and the atomic encounter aggregate now pass N=1–8 scaling, authored ticks, break damage, contribution, lethal cleanup, rollback, schemas, mutation, and CI validation. The complete real-system solo fixture defeats at exactly 180 seconds and repeats with an identical BLAKE3 trace. Implement stable reward/exit identities, SOC-010 eligibility, durable personal reward terminality, and reward-gated exit creation next; extraction and later layers remain fail closed. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
