...
[`GB-M03-04F`](docs/milestones/GB-M03-04F-audit.md) is complete under its three-authority audit. [`GB-M03-04G`](docs/tasks/GB-M03-04G.md) now has green hosted proof for append-only protocol 1.12, authenticated dispatch, positive disposable feature negotiation, accepted CharacterSafe-to-Vault transfer, exact same-connection replay, and replay through a newly bound real-QUIC/PostgreSQL endpoint; the normal Core runtime remains nonadvertising and disabled. Require the expanded altered-payload rejection on both endpoints, then add a canonical `04A`-`04F` lifecycle signature and compare it across reconnect/restart before closing the adverse, visual, performance, and parent-audit gates. Keep death/extraction/Recall, Overflow/ResolutionHold, salvage/crafting, Core promotion, Character Select `Play`, Realm Gate and Vault station admission, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
