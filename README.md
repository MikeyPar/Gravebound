...
> **Project status:** M01 and M02 are closed under their recorded gates. M03 has closed identity, PostgreSQL foundations, exact world-flow content, atomic dormant transfers, Hall/private-microrealm simulation and native evidence, progression through durable reward placement, the complete first Oath/Bargain package, and the minimal Ash wallet under their recorded audits. `GB-M03-03D` now compiles the exact six-normal/two-miniboss behavior roster, strict hashed patterns and derived warning cues, deterministic room rotation and navigation, and the branch-disabled B0-to-B6 corridor chain. Parent `GB-M03-02`, `03`, and `04` remain open alongside permanent death, successor recovery, extraction/Recall, telemetry, support, Echo, platform, and final private-loop gates.
...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now compiles all eight roster actors without constructing runtime entities: three immutable First Playable simulation definitions plus five exact Core-authored definitions and their complete pattern/cue bindings. Convert the authored millisecond contracts into 30 Hz deterministic simulation definitions next, then prove target selection, state boundaries, scheduler maxima, reset behavior, and minimum-speed counterplay before constructing encounters. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as their dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
