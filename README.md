...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice now has a schema-`0039` serializable death/Echo repository (`ae7427c`) and a mandatory hosted atomicity gate (`3b87bc1`). The transaction binds promoted content, terminal clocks and aggregate versions, destroys exact at-risk item/material custody, clears Oath/Bargain life state, archives the dead character, stores death/trace/summary/memorial/receipt/audit/outbox state, and creates/promotes eligible Echoes under one deferred closure. Require exact-head hosted run `29389259898`, then append protocol `1.13` kind `18`, add authenticated read dispatch, finish `06B` authority and the remaining `06C` arbiter/Bell seams, and prove real-QUIC restart/replay before joint audits. Keep all player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal-route behavior fail closed.
