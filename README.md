...
[`GB-M03-04F`](docs/milestones/GB-M03-04F-audit.md) is complete under its three-authority audit. [`GB-M03-04G`](docs/tasks/GB-M03-04G.md) now has green hosted proof for append-only protocol 1.12, authenticated dispatch, accepted CharacterSafe-to-Vault transfer, exact replay, and altered-payload conflict through initial and newly bound real-QUIC/PostgreSQL endpoints; the normal Core runtime remains nonadvertising and disabled. Require hosted proof for the expanded disposable journey and its complete typed signature: starter, Caldus XP/first-clear, deterministic reward entries, field-equipment confirmation, safe transfer, complete item/ground/destruction identity, custody, slot/stack and ledger chains, typed stale/foreign authority rejections, exact replays/conflict, disconnect-after-commit retry on a fresh endpoint, concurrent duplicate-session convergence, exact Core item-content enforcement, closed-database state-free rejection, structural receipt corruption, semantic replay-hash corruption, persistent projections, a nonmutating Realm Gate rejection on every endpoint, and byte-identical reconnect/restart state. The same hosted gate must complete 25 measured journeys below the 30-second median login budget with no idle transactions. Then capture/hash/inspect the optimized native lifecycle matrix and publish the `04G`, parent `04`, and `02C` audits. Keep death/extraction/Recall, Overflow/ResolutionHold, salvage/crafting, Core promotion, Character Select `Play`, Realm Gate and Vault station admission, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
