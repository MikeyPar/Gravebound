...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now compiles all eight roster actors without constructing runtime entities and proves every Core-authored COM-006 route from those definitions, including fan arrival, charge escape, radial gaps, rotor tracking, close-spawn ground warning, and minimum projectile arrival. Implement deterministic target selection, telegraph/aim locks, state transitions, leash/no-target reset, and `pack.bell.01` encounter construction next. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as their dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
