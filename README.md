...
> **Project status:** M01 and M02 are closed under their recorded gates. M03 identity, PostgreSQL foundations, exact world-flow content, atomic dormant transfers, Hall/private-microrealm simulation and native evidence, progression through durable reward placement, the complete first Oath/Bargain package, and the minimal Ash wallet pass under their recorded audits. `GB-M03-03D` now has strict, hashed Core encounter/room records plus deterministic room rotation, navigation, and B0→B6 corridor compilation; exact behavior records are active next. Parent `GB-M03-02`, `03`, and `04` remain open alongside permanent death, successor recovery, extraction/Recall, telemetry, support, Echo, platform, and final private-loop gates.
...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` strict roster/templates/pack/layout now compile into deterministic rotated room geometry, collision-aware navigation, and the branch-disabled B0→B6 corridor chain. Add typed exact normal/miniboss behavior and pattern records with fixed counterplay fixtures next, then construct encounters and continue `03E`-`03F`. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as their dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
