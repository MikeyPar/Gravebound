...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now compiles all eight roster actors, proves every Core-authored COM-006 route, owns the shared enemy lifecycle and all five Core-authored kit schedulers, atomically constructs exact `pack.bell.01` assignments, and instantiates them through the existing authoritative normal-wave runtime on the shared warning boundary. Connect authoritative wave completion and five-second empty reset to the private-microrealm lifecycle without opening the normal route. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as their dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
