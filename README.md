...
Commits `f4ad323`, `e5f7dc8`, `1bd230a`, and `47ad6c3` add the session-owned automatic Caldus executor, coherent post-reward PostgreSQL custody snapshot, append-only protocol 1.19 pending-inventory projection, and transport-independent extraction binding. See the [extraction-prerequisite evidence](docs/evidence/GB-M03-03G-caldus-extraction-prerequisites-evidence.md). Next publish and retain that snapshot from the Caldus worker, construct/register the production extraction actor only from the exact committed `BossExitReady` result, and prove active/reconnected/`LinkLost` behavior. Then run hosted PostgreSQL/real-QUIC adverse and restart proof. Bound normal-server construction and normal admission remain disabled.
