...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Atomic death/Memorial/Echo has real-QUIC/PostgreSQL restart, replay, adverse, latency, soak, reachable-branch, and distinct-account concurrency evidence. The server-owned `06C` planner now converts raw custody into exact Equipment, Belt, stacked RunBackpack, PersonalGround, and run-material destruction without caller-authored ordinals, post-versions, or ledger IDs; [`SPEC-CONFLICT-028`](docs/spec-conflicts/SPEC-CONFLICT-028-m03-runbackpack-unit-destruction-order.md) records the stacked-unit tie-break. The repository rejects Safe Equipment/Belt corruption and preserves real CharacterSafe/Vault rows. Hosted run `29500484938` correctly rejected the first full-custody fixture because CharacterSafe was seeded before its inventory owner; the fixture now respects custody creation order. A direct 17-family matrix also attempts 34 production-triggered UPDATE/DELETE mutations and requires the canonical terminal signature plus exact replay to remain unchanged. Death closure remains open for hosted acceptance of those fixes, typed post-death command proof, and optimized native capture. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
