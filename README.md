...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **79% complete**: identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete Core item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, successful extraction/Emergency Recall/ResolutionHold, and successor recovery are closed under hosted evidence. [`GB-M03-08`](docs/milestones/GB-M03-08-audit.md) passed at commit `2535e8c` on hosted CI [`29554811453`](https://github.com/MikeyPar/Gravebound/actions/runs/29554811453). Successor recovery has append-only protocol `1.17`, reliable kind `23`, `core_successor_v1`, pinned `1.16` compatibility, migrations `0060`/`0061`, universal death presets/reservations, the shared transaction-local exact `04D` starter seam, a replay-first atomic writer, authenticated fail-closed service, and durable response-loss/reconnect/pool/OS-process replay. Its native authority gates Create Successor on the opaque durable terminal summary, retains exact retry, preselects the legal level-one successor, and treats Play as confirmation two behind matching Hall readiness. The optimized 32-frame resolution/effects/scale matrix passes. Hosted CI [`29608793943`](https://github.com/MikeyPar/Gravebound/actions/runs/29608793943) passes the cumulative Linux/Windows/PostgreSQL/native matrix and the hashed 25-journey route: median death-to-control `118.676 ms`, p95 `129.126 ms`, 25/25 checked danger returns, 25 unique successors, 100 unique starter items, exact durable graphs, and zero operational residue. [`GB-M03-07`](docs/milestones/GB-M03-07-audit.md) is closed. Exact source `037efe6` is green under hosted CI [`29630076008`](https://github.com/MikeyPar/Gravebound/actions/runs/29630076008) for the terminal-first bootstrap, schema-63 extraction-intent persistence, route-owned `TerminalPending` permit, live Caldus intent actor/planner, response-loss/reconnect replay, Linux quality/content/schema checks, PostgreSQL transactions, Windows release construction, and optimized native evidence. Commit `26c420b` adds the dormant all-or-nothing persistent private-life foundation; commit `551c527` adds the locally green per-account shared-writer transport session with dynamic Recall bind/rebind/unbind and zero-residue shutdown. Dynamic extraction, transition/bootstrap reconciliation, the live private-route graph, ordinary native route, telemetry, support, hosting/platform evidence, and final private-loop/cohort gates remain open. Normal route admission is still disabled.
...
The three-authority parent audit accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). Exact source `037efe6` is green under hosted CI [`29630076008`](https://github.com/MikeyPar/Gravebound/actions/runs/29630076008) for the terminal-first bootstrap, schema-63 durable extraction-intent acceptance/conflict audit, route-owned `TerminalPending` permit, paired route/world content authority, the live Caldus extraction actor/planner, response-loss/reconnect replay, PostgreSQL transactions, strict Linux quality/content/schema checks, optimized Windows construction, and native evidence. Commit `26c420b` constructs exact Core content and the dormant reusable persistent authority graph. Commit `551c527` owns one monotonic transport generation and shared reliable writer per authenticated account from handshake onward; Recall binds only with a danger actor, rebinds before handoff visibility, preserves LinkLost, ignores stale teardown, and retires independently without closing the Hall writer. Local formatting, 2/2 session tests, 4/4 Recall tests, strict all-target server Clippy, and the complete 283-test server suite pass. Next, implement the account/generation-keyed dynamic extraction runtime on the same writer, including committed-result publication and cleanup; then add post-commit transition reconciliation and terminal-first runtime bootstrap. Keep `core_world_flow_integration`, Character Select `Play`, Realm Gate interaction, and normal extraction/Recall admission disabled until live movement, combat, rewards, pending inventory, all terminal owners, real-QUIC restart/25-journey evidence, and visual review pass. Keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed. The unregistered Sir Caldus [attack-motion](assets/core/bosses/sir_caldus/review/v2/README.md), [idle/recovery](assets/core/bosses/sir_caldus/review/v3/README.md), and [Bell Ring](assets/core/bosses/sir_caldus/review/v4/README.md) review packs remain outside runtime hashes pending combined in-engine timing, attack-origin/hurtbox/collision, anchor-drift, and optimized native review.
