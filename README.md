...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schemas `0043`–`0046` define normalized live `06B` clocks, v2 deed receipts, the bounded ten-second trace graph, and complete crash-time deed revocation. The serializable deed repository derives exact Sir Caldus/Sepulcher authority from locked terminal reward and XP evidence, binds the selected living danger character, immutable entry root, active lineage, promoted content revisions, character version, and life-metrics CAS, and records canonical replay/conflict evidence. The crash writer now revokes the complete root-bound deed and XP set, rebuilds the latest nonrevoked projection, and stores canonical per-change, set, and projection digests under a mandatory contract-1 sidecar, including zero deeds; historical replay is reconstructed by life-metrics versions rather than timestamps. Comprehensive hosted PostgreSQL fixtures are registered in CI, but exact-head execution is still required before closure. Next obtain that proof, then complete clock/trace persistence and server ingestion, wire the five-producer terminal coordinator to execution/recovery, and prove committed death through real QUIC with response loss and restart. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
