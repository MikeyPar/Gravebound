...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice now has the mandatory schema-`0049` retained-live-trace promotion graph in the same serializable death/destruction/Memorial/Echo transaction; schema `0050` supplies bounded provenance diagnostics. Commits `2253d89`/`aa51f27` require the sealed promotion, and `8809c98`/`b9d53b9` preserve, revalidate, and clean its source graph across commit, restart replay, and rollback. Commits `be7f163`/`89e8159` bind exact immutable views and prove abandoned real-QUIC delivery plus full process-authority restart. Commit `21fa8a4` restores canonical Safe-versus-`AtRiskEquipped` combat custody reads. Commits `9f1b37f`/`6a3c1e6` compose all five same-tick terminal producers through the death writer and prove seven participant rollbacks plus deterministic serialization recovery. Commit `d2a3db2` closes the actual PostgreSQL-pool outage, blocked-departure, reconnect, and identical prepared-winner convergence case. Local focused tests and strict Clippy are green; exact-head CI `29432319053` is queued. Next obtain hosted proof, then deliver the `06D` durable-acknowledged death summary and Memorial Wall before the joint `02D`/`06`/`13` audits. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
