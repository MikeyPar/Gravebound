...
Source `572318f` is packaged as [optimized tester r38](docs/evidence/GB-M03-tester-r38-manifest.md). Its primary launcher carries pinned PostgreSQL 17.10, starts it on loopback, preserves wipeable data under `.runtime`, and reaches the real schema-79 server and native private-life client without Docker. Commits `b2d31ce` and `ebef5e8` make the terminal-view and gameplay-render queries disjoint, removing both reported Bevy `B0001` startup panics; publication now exercises that exact persistent client path. Hosted run [`30069213403`](https://github.com/MikeyPar/Gravebound/actions/runs/30069213403) passed schema-79/pre-checkpoint proof and reached tick 1300 before the evidence driver reused primary-press ID `1`; harness correction `87a7967` is under corrected hosted run [`30071229521`](https://github.com/MikeyPar/Gravebound/actions/runs/30071229521). The exact-25 evidence compiler exists at commit `886387f`; it does not claim 25 completed private loops yet. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is hosted green single-route completion, then 25 complete ordinary loops, current aggregate timing, optimized Realm Gate capture, and bounded operational telemetry proof. Private-cohort comprehension, backup/restore, hosting, destination/privacy approval, and Steamworks evidence remain required. Formal progress remains **15/23 (65%)** until a whole package or exit outcome closes.
