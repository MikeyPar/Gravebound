...
[`GB-M03-03E`](docs/tasks/GB-M03-03E.md) is active under the three-authority contract and approved `SPEC-CONFLICT-022`–`024`. Strict content, DNG-006 lock/reset, exact Caldus combat, and the atomic encounter pass their current gates; the complete real-system solo fixture defeats at exactly 180 seconds and repeats with an identical BLAKE3 trace. Domain-separated encounter/reward/exit identities and exact SOC-010 eligibility now pass retry, nonreuse, owner-order, contribution, presence, inactivity, Recall, session, and anti-cheat boundaries. Implement durable exact Core reward/progression commits and reward-gated stable exit creation next; extraction and later layers remain fail closed. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
