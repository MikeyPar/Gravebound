...
[`GB-M03-03E`](docs/tasks/GB-M03-03E.md) is active under the three-authority contract and approved `SPEC-CONFLICT-022`–`024`. Strict Caldus/exit content, DNG-006 lock/reset, the exact scheduler, shared-hostile projectiles, fixed-point Charge Lane body/contact/center return, real scaled health/armor/hurtbox/friendly damage, and the atomic encounter aggregate now pass N=1–8 scaling, authored ticks, target/gap cursors, break damage, contribution, lethal terminality, cleanup, overkill, low-health, soft-enrage, cross-system contact, rollback, schemas, mutation, and CI validation. Add deterministic all-phase 150–210 second combat/replay fixtures next, then durable reward and stable exit closure; extraction and later layers remain fail closed. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
