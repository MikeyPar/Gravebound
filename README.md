...
Commits through `5b69b5b` complete the ordinary native Core route controls for Bell transfer, fixed-room doors, the durable B4 Bargain, Emergency Recall, and server-authorized B6 extraction. Extraction readiness is append-only protocol authority correlated to the exact pending-inventory, lineage, restore point, content revision, and aggregate versions; exact retries reuse one retained mutation, and Hall control is installed only after the stored result reaches the current reliable transport. This follows the canonical GDD terminal rules, the Content Production Spec's fixed `B0 -> B6` route, and the Development Roadmap `GB-M03-03`/`GB-M03-08` gates. The Current Next Step is to reconcile stored lethal-death summary, Memorial, Hall, and successor creation in the same native route, then finish telemetry, support lookup, hosting/platform packaging, the newest tester build, and the user-deferred full audit/external gates.
