...
[`GB-M03-03E`](docs/tasks/GB-M03-03E.md) is active under the three-authority contract and approved `SPEC-CONFLICT-022`–`024`. Strict content, DNG-006 lock/reset, exact combat, 180-second replay, stable encounter/reward/exit/extraction identities, and shared SOC-010 eligibility pass their current gates. Schema 25 and the victory coordinator close exact personal rewards before the lineage/attempt-bound exit can appear; schema 26 owns only the wipeable typed extraction receipt and atomically supersedes crash restore without touching GB-M03-08 item conversion. Protocol 1.11 and the dedicated wipeable Hall coordinator now bind the two extraction identities, exact danger lineage/restore point, idempotent `HallDefault` transfer, checkpoint cleanup, late-failure rollback, database-pool restart replay, crash-restore race ordering, and fail-closed payload/content preflight while leaving inventory unchanged. Produce the native evidence, run cumulative gates, and write the 03E completion audit next. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
