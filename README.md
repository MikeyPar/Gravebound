...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Add a forward-only migration that splits the successor result, audit, and outbox insert-window functions so relation-specific fields are resolved only against their owning row type; then re-run the mandatory hosted `postgres_successor` gate. The authenticated service is locally green and should enter only the disposable real-QUIC route after that proof. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
