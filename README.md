...
[`GB-M03-07`](docs/milestones/GB-M03-07-audit.md) is complete on hosted CI [`29608793943`](https://github.com/MikeyPar/Gravebound/actions/runs/29608793943), including the [immutable 25-journey timing/identity/residue report](docs/evidence/GB-M03-07-successor-recovery-manifest.md). Integrate and audit parent `GB-M03-03` next: enable only the production Character Select → Hall → micro-realm → dungeon → boss → extraction/death → Hall/successor route gates whose owning packages now pass. Keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed.
