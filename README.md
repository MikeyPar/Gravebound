...
[`GB-M03-03C`](docs/milestones/GB-M03-03C-audit.md) closes exact Hall/private-microrealm simulation, inspected standard/reduced-motion presentation, and the pinned interaction/lifecycle trace. `GB-M03-03D` now owns the exact `pack.bell.01` warning, deterministic construction, authoritative combat, terminal clear, and five-second empty reset in addition to the compiled roster/shared AI/kit schedulers. Implement reusable fixed-room authority for B1, B2, B3, and B5 next, including warnings, completion/quiet cleanup, reset, door safety, and reward/XP seams. Continue `GB-M03-04E`-`04G` and the approved `SPEC-CONFLICT-009` death foundation as their dependencies become ready. Keep 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, the normal player route, and affected Hall stations fail closed until their owning gates pass.
