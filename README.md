...
[`GB-M03-04F`](docs/milestones/GB-M03-04F-audit.md) is complete under its three-authority audit: schema-28 CharacterSafe/Vault custody, the exact 8/160/8 planner, normalized schema-29/30 serializable transfers, bounded dormant coordination, and all-or-nothing danger-entry auto-deposit pass the full hosted PostgreSQL/release/quality matrix. [`GB-M03-04G`](docs/tasks/GB-M03-04G.md) is active: append the safe-inventory request/result to the reliable protocol and persistent authenticated dispatch, then prove the complete `04A`-`04F` item/Vault lifecycle through disposable real QUIC, reconnect, restart, adverse, visual, and performance gates before publishing the `GB-M03-04`/`GB-M03-02C` parent audits. Keep death/extraction/Recall, Overflow/ResolutionHold, salvage/crafting, Core promotion, Character Select `Play`, Realm Gate and Vault station admission, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
