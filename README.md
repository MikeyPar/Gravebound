...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice now has the mandatory schema-`0049` retained-live-trace promotion graph in the same serializable death/destruction/Memorial/Echo transaction; schema `0050` supplies bounded provenance diagnostics. Commits `2253d89`/`aa51f27` require the sealed promotion, and `8809c98`/`b9d53b9` preserve, revalidate, and clean its source graph across commit, restart replay, and rollback. Commit `be7f163` binds immutable death views to the exact canonical revision written by that transaction. Commit `89e8159` adds the server-authored production-path death fixture and authenticated real-QUIC proof: it abandons a response, recreates the PostgreSQL pool, repositories, arbiter, TLS endpoints, and connection, replays the original terminal result, reads latest/summary/Memorial/trace projections, and rejects duplicate domain state without exposing a client lethal command. Local focused tests and strict Clippy are green; exact-head CI `29430609063` is running. Next obtain that hosted proof, add same-tick and boundary-injected adverse evidence, and deliver the durable-acknowledged death summary and Memorial Wall before the joint `02D`/`06`/`13` audits. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
