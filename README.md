...
Source `503bb1a` is packaged as optimized tester r22 with the assembled native route, full-route accessibility controls, and forward schema-69 item authority. Client/server release construction, strict content validation, CLI checks, a 12-second Local Lab launch, and all seven standalone presentation modes passed; publication then atomically removed every stale `dist` artifact. Native mutation IDs remain nonzero UUIDv7 values across restart, and open danger clocks remain exact while closed terminal roots fail closed. `GB-M03-10` is complete under hosted PostgreSQL least-privilege proof. Schema 70 has optional production-root logical-session wiring with accepted-client platform, explicit environment/region, durable-head recovery, generation-safe reconnect, clean exit, bounded failure containment, and no gameplay dependency. Its synthetic first-combat fixture now stages the complete lineage, restore root, and five snapshot components atomically. The real-QUIC production-root journey reaches controllable Hall, traverses the authored Hall, opens the Realm Gate, commits the portal mutation, reaches matching controllable Core-microrealm authority, and requires exact SessionStarted/AccountCreated/CharacterCreated/CharacterEnteredCombat/CleanExit facts before cleanup. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is that corrected hosted schema-70/production-root gate; then B0-B6, Caldus, extraction, death/successor, 25 loops, crash collection, loot origin binding, and the disabled exporter worker. Current optimized Realm Gate capture, private cohort, comprehension metrics, backup/restore, hosting, and Steamworks proof remain required gates.
