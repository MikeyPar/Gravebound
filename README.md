...
Commits through `87debaf` add the dedicated normal server/client route and continuously publish committed driver route changes through one generation/version-deduplicated writer. Generic observation cannot overtake the ordered B3 progression/route or Caldus pending-inventory/route bundles. The Current Next Step is a server-owned `CorePrivateGameplayObservation` projected inside each authoritative runtime with complete player/enemy/boss/projectile state, source identities, tick, route version, and acknowledged input; send its latest state at 15 Hz and consume it only after matching reliable route authority. Then finish stored extraction/Recall/death destination reconciliation and interaction dispatch before telemetry, support lookup, hosting/platform evidence, and the user-deferred audit/external gates.
