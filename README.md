...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **98% complete**. Identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, extraction/Emergency Recall/ResolutionHold, and successor recovery are closed under hosted evidence. The normal private-life path now has a dedicated terminal-first persistent server, authenticated reliable/datagram dispatch, authoritative Hall/Realm Gate admission, shared-writer route publication, and a negotiated native Character Select -> Hall -> danger client state. Remaining work is continuous gameplay/snapshot publication and terminal-to-destination client reconciliation, then telemetry/support/hosting and the final hosted/private-cohort/platform gates.
...
Commits through `dc76ce7` complete the ordinary native Core route through extraction or lethal death, Memorial and successor recovery; add privacy-safe committed-outbox telemetry and authenticated bounded support lookup; enforce server-owned Hall station range/hold/panel authority; integrate the initial Oath Shrine and forced ResolutionHold recovery; and deliver durable Q/E Belt consumables with exact replay, server-owned quantities, live cooldown/full-health preflight, tonic feedback, and atomic reservation against terminal outcomes. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is to add usable paged Vault/Overflow inspection and bounded Overflow withdrawal, complete production-blocking combat presentation, then build the newest tester and remove stale `dist` artifacts. The owner has deferred the exhaustive audit, broad test matrix, performance/visual evidence sweep, cohort metric, and external Steam/platform evidence until implementation is complete.
