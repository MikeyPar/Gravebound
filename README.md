...
The three-authority parent audit identified the missing normal composition root and accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). [`GB-M03-03G`](docs/tasks/GB-M03-03G.md) slice 1 is green at exact source `26f9c66` under hosted CI [`29616169514`](https://github.com/MikeyPar/Gravebound/actions/runs/29616169514). Append-only protocol `1.18` `CorePrivateRouteStateV1` and the room-aware native control projection are implemented locally at `5e1c5f9`/`eae98f4`: full composed-route content binding, pinned 1.17 bytes, closed Hall/micro-realm/B0-B6/Caldus phases, typed readiness, generation/state replay rules, location-event arrival buffering, exact scene readiness, and LinkLost/terminal/stale-generation revocation pass strict lint and the cumulative runnable workspace suite. Run hosted cumulative proof next; after green, construct the live capacity-one actor and dedicated `BoundCorePrivateLifeServer`. Keep normal capability advertisement, Character Select `Play`, and Realm Gate interaction disabled until the live actor, terminal owners, ordinary native route, and cumulative evidence are attached; keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed.
