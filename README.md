...
Commits `3f749dd`, `e7905ea`, `e62587b`, and `6d4fed6` cover the complete local `CaldusRewardPending` worker through durable resolution, production extraction activation, retained publication, `LinkLost`/reconnect, terminal release, route reopening, and no resurrection; see the [activation evidence](docs/evidence/GB-M03-03G-caldus-extraction-activation-evidence.md). Commit `467b313` adds the [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md): Recall and extraction now read one post-commit driver clock through the exact route lease, reject tick zero and stale generations, and remove authority only after joined danger teardown. Hosted run [`29667827330`](https://github.com/MikeyPar/Gravebound/actions/runs/29667827330) proves Windows release, all B3 cases, and all eight Caldus PostgreSQL cases green; its remaining `04G` failure was a globally reused fixture reward ID, isolated by `3da08c5`. Next obtain a fully green hosted run for this cumulative head, then compose live lethal death, the five-producer terminal loop, danger activation, and ordinary dispatch before normal admission opens.
