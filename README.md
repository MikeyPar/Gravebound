...
The Caldus-to-extraction worker and [live authoritative tick contract](docs/evidence/GB-M03-03G-live-authoritative-tick-evidence.md) are green through hosted run [`29668693282`](https://github.com/MikeyPar/Gravebound/actions/runs/29668693282). Commits through `c9f8705` retain the lossless private-route event stream. Commits `2e02b94` and `0991cd6` add the [opaque danger authority](docs/evidence/GB-M03-03G-danger-entry-authority-evidence.md) and [mandatory owner lifecycle](docs/evidence/GB-M03-03G-terminal-owner-lifecycle-evidence.md). Commit `8107752` adds [durable live-trace activation and immutable entity identities](docs/evidence/GB-M03-03G-live-trace-activation-evidence.md), removing the need to guess checkpoint ticks or current versions. Commit `e869f01` adds [same-boundary terminal tick context](docs/evidence/GB-M03-03G-terminal-tick-context-evidence.md) across micro-realm, fixed dungeon, and Caldus frames, including lossless `LinkLost` and one acknowledged `Reattached` frame. Next build the PostgreSQL death-context planner and server adapter, then connect clocks, trace, Recall phase, durable lethal acknowledgement, and all five terminal producers; normal admission remains fail closed until that factory exists.
