...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice now has schema `0042`, including account-bound Echo promotion triggers, the serializable death/Echo writer, immutable account-bound death/summary/Memorial/trace readers (`3d11b85`), protocol `1.13` kind `18`, and authenticated fail-closed server dispatch (`6bd9e66`). The shared server-owned terminal arbiter now gives death absolute same-tick precedence, binds the active lineage and restore point, blocks unresolved departures, and reconstructs versioned receipts after restart. Next connect the pure `06B` evidence envelope to that arbiter and the single terminal writer, then capture committed-death real-QUIC restart/replay before the `06A`/`02D` audit. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
