...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md) slice now has forward schema `0034` and locally green V3 capture: all five safe aggregates, exact item identity/provenance, pre-entry Backpack, consumed-Belt identity authority, Bargain provenance, life clocks, Ash authority, and immutable crash-result storage. The schema-33 hosted run exposed a typed test bind and immediate cleanup FK; `0034` repairs both forward-only. Accepted [`SPEC-CONFLICT-027`](docs/spec-conflicts/SPEC-CONFLICT-027-m03-crash-restore-completeness.md) defines completeness, and correction [`SPEC-CONFLICT-028`](docs/spec-conflicts/SPEC-CONFLICT-028-m03-crash-baseline-identity-capacity.md) fixes the bounded legal maximum at 64 durable identities without changing slot or stack capacities. Require exact-head hosted PostgreSQL proof for schema `0034`, then implement and prove the single all-component crash-restore transaction before starting the strict death repository/protocol. Keep all player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal-route behavior fail closed.
