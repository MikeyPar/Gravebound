...
> **Project status:** M01 and M02 are closed. The current GB-M03 player route is assembled in optimized tester r22, while **15 of 23 fixed formal closure units are proven (65%)**. The stable denominator is 14 Roadmap work packages plus nine explicit M03 exit outcomes; partial gates do not count. The open engineering work is final production-route/25-journey proof and committed-source coverage for the remaining telemetry families. The private cohort, hosting/backup rehearsal, and Steamworks evidence remain external gates. See the [formal gate audit](docs/milestones/GB-M03-gate-audit.md), [support completion audit](docs/milestones/GB-M03-10-audit.md), and [r22 release manifest](docs/evidence/GB-M03-tester-r22-manifest.md).
...
| [M03 Formal Gate Audit](docs/milestones/GB-M03-gate-audit.md) | Fixed-denominator closure status, evidence boundaries, and remaining engineering/external gates |
...
The native route is compiled from the exact Core world records and localization. Fixed-point collision/navigation, server-owned interaction projection, camera bounds, standard/reduced-motion presentation, and the Dormant -> Waiting -> Active -> Cleared lifecycle remain deterministic. Three stable object IDs resolve to hash-locked presentation art while their compiled geometry remains visible and authoritative; see the [visual manifest](docs/evidence/GB-M03-03G-microrealm-landmark-visual-manifest.md). The persistent live owner accepts action state only and derives its own frame tick, equipment-speed displacement, combat/collision, exact Bell wave, clear projection, and Slipstep under one rollback-safe route compare-and-swap; see the [combat](docs/evidence/GB-M03-03G-live-microrealm-combat-evidence.md) and [Slipstep evidence](docs/evidence/GB-M03-03G-live-slipstep-evidence.md). The retained owner runs on an exclusive independent 30 Hz driver across `LinkLost` and reconnect; see the [driver evidence](docs/evidence/GB-M03-03G-independent-microrealm-driver-evidence.md). The normal authenticated route now composes this foundation through fixed B0-B6 combat, rewards, pending inventory, and every terminal producer.

### GB-M03 authoritative combat presentation

![Sepulcher Knight production scale and silhouette review](assets/core/enemies/sepulcher_knight/v1/previews/sepulcher-knight-scale-review.png)

Protocol 1.23 binds every player, enemy, and boss identity to the exact route generation, content revision, room, and state version. Reliable telegraph starts carry fixed-size, server-authored fan, lane, ring, and rotor geometry plus explicit Physical/Veil presentation; the client neither infers geometry from names nor renders danger actors before the matching binding arrives. The native playfield now uses the Grave Arbalist, all six Core normal enemies, the Sepulcher Knight, and Sir Caldus with a compact health/boss/objective/Belt/Recall HUD, exact 35%/15% health warnings, reduced-motion timing parity, high-contrast telegraphs, and an F6 accessibility panel. The image above is the asset-scale readability review, not a simulated gameplay result; the focused [combat-presentation integration evidence](docs/evidence/GB-M03-03G-combat-presentation-integration.md) records the authority and verification boundary. Optimized source revision `503bb1a` is packaged in [tester r22](docs/evidence/GB-M03-tester-r22-manifest.md) for native route review.
...
Source `33ee53d` is packaged as optimized tester r24 with the assembled native route, full-route accessibility controls, and schemas through 75. Client/server release construction, strict content validation, CLI checks, a 12-second Local Lab launch, and all seven standalone presentation modes passed; publication atomically removed every older `dist` artifact. `GB-M03-10` is complete under hosted PostgreSQL least-privilege proof. The real-QUIC production-root source journey now uses only public protocol and ordinary gameplay input through microrealm combat, authored Bell travel, B0-B6, the committed B3 reward, authoritative B4 `NoOffer`, Sir Caldus, durable extraction, server-planned pending-item placement, and receipt-bound Hall return. The same session then re-enters danger, receives server-authored lethal damage, reads the durable death summary/Memorial/trace, creates one server-planned successor, and proves checked native control in Hall and a fresh danger route. Protocol 1.24 adds negotiated privacy-safe next-launch panic reporting. Schemas 71/72 bind new item-lifecycle facts without making telemetry a gameplay dependency; hosted run [`29907197863`](https://github.com/MikeyPar/Gravebound/actions/runs/29907197863) applied both schemas and passed both real loot-source journeys. Schema 73 binds the four terminal families to immutable PostgreSQL-clock session origins and derives session duration and item-power band from committed facts; hosted run [`29913145208`](https://github.com/MikeyPar/Gravebound/actions/runs/29913145208) passed its database-clock resolver before isolating a shared test-only `bytea` fixture defect. The identity transaction establishes every new account's required zero-balance Ash wallet, and additive schema 75 corrects schema 74's content-label/namespace mismatch for pre-existing wipeable accounts without rewriting migration history or changing existing wallets. The production private-life root owns a typed disabled telemetry worker with no source, exporter, secret, task, poll, acknowledgement, queue activity, or gameplay dependency. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is hosted proof for all four schema-73 terminal families, schema 75, and one complete extraction/death/successor route, then the remaining `TEL-003` boss, party/contribution, and network-health fields. After one complete route is green, run 25 loops and current aggregate timing. Current optimized Realm Gate capture, private cohort, comprehension metrics, backup/restore, hosting, and Steamworks proof remain required gates. Formal progress remains **15/23 (65%)** until a whole package or exit outcome closes.
