...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schemas `0043`–`0047` now define the normalized `06B` graph, v2 deed receipts, bounded ten-second trace, crash-time deed revocation, and the contract-1 authoritative clock command. The serializable clock repository owns all eight 30 Hz states, derives clock/link-loss/version results, binds selected living character and exact active danger authority, enforces contiguous intervals and the 90-tick LinkLost boundary, records changed-payload audits, and reconstructs a strict restart head. The deed/crash writers continue to derive exact reward authority, revoke the complete root-bound deed/XP set, and rebuild version-ordered projections. Unit, strict-Clippy, and hosted-fixture construction gates are green locally, including fail-closed restart corruption and a real clock-to-crash rollback path; exact-head PostgreSQL execution remains required before closure. Next obtain that proof, add retained trace-ingest receipts and live trace/server ingestion, then wire the five-producer terminal coordinator to execution/recovery and prove committed death through real QUIC with response loss and restart. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
