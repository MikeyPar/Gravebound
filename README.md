...
Commits `3f749dd` and `e7905ea` now reserve the exact `BossExitReady` route before the final custody read, preserve storage versions separately from the wire projection, construct/register the production extraction actor, bind it to the existing private-life writer, and prove the automatic post-acknowledgement order through activation. Fresh/replayed ownership prevents early, exact, or altered replay from aborting the winner; fail-closed cleanup, terminal-completion release, and unresolved-owner teardown gates are covered in the [activation evidence](docs/evidence/GB-M03-03G-caldus-extraction-activation-evidence.md). Next drive the full live worker from `CaldusRewardPending` through durable resolution/acknowledgement and prove active transport, response loss, reconnect, transport-free `LinkLost`, competing terminal, completion release, shutdown, and zero residue. Then run hosted PostgreSQL/real-QUIC adverse and restart proof. Bound normal-server construction and normal admission remain disabled.
