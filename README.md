...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Under the canonical GDD, Content Production Spec, and Development Roadmap, commits `fb8f56a` and `9095cf7` define and load the versioned canonical death/destruction/Memorial/Echo terminal signature inside one serializable read-only PostgreSQL snapshot. Hosted CI `29478501170` proves exact signature equality across a fresh commit, a new database connection, and exact replay; 179 persistence unit tests, strict lint/content gates, the mandatory PostgreSQL gate, and the Windows release build are green. The active [`GB-M03-06E`](docs/tasks/GB-M03-06E.md) step is authenticated real-QUIC response-loss and newly bound server-process equality, followed by the summary/Memorial/destruction/Echo/cleanup/outbox adverse matrix, concurrent lethal writers, rollback, performance, cleanup, and the joint `06`/`02D`/`13` audits. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
