...
Latest M03 authority-state review (static review mock, not native capture):

<img src="assets/core/ui/caldus_resolution_states/v1/previews/caldus-resolution-states.standard.1920x1080.review-mock.png" alt="Sir Caldus defeated reward-unresolved and reward-committed-at-risk authority state review" width="960">
...
Commits `83ccbb1`, `83c75a5`, `3f4ecaf`, and `8a4898d` implement the durable B3 authority, corrected terminal semantics, transport-independent executor, and one canonical production loader for PostgreSQL, progression, Oath/Bargain, reward content, and a caller-owned process epoch. Malformed authority fails closed, SOC-010 ineligibility commits no grant, exact acknowledgement never re-pends, progression precedes route publication, transient database errors retry without transport, and the retained result replays contiguously to each newer reliable-writer generation; see the [three-authority evidence](docs/evidence/GB-M03-03G-durable-b3-reward-authority-evidence.md). Next construct the bound normal private-life server root, read `SecretRewardEpoch` once there, inject the canonical loader into its shared session directory, and prove hosted inactivity zero-row, response-loss, reconnect, and process-restart convergence. Then complete B5 and authoritative Sir Caldus B6. Normal admission remains disabled.
