...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice now has schema `0042`, including account-bound Echo promotion triggers, the serializable death/Echo writer, immutable account-bound death/summary/Memorial/trace readers (`3d11b85`), protocol `1.13` kind `18`, and authenticated fail-closed server dispatch (`6bd9e66`). The shared terminal arbiter gives death absolute same-tick precedence and reconstructs versioned receipts after restart; the server death builder seals authenticated `06B` evidence, and the execution adapter now permits only that prepared lethal winner to call PostgreSQL, validates the database-authored committed result byte-for-byte after timestamp rebinding, and keeps departure blocked on failure. Next prove this complete path through hosted PostgreSQL and committed-death real QUIC with response loss/restart before the `06A`/`02D` audit. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
