...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schemas `0043`–`0048` now define normalized live evidence, v2 deed receipts, crash-time deed revocation, the contract-1 authoritative clock command, and retained live-trace ingestion. The trace writer accepts only complete server-authored nonlethal ticks for the selected living danger character, preserves simulation and durable entity identity, prunes normalized payload at the exact 300-tick boundary while retaining replay receipts, rejects changed payloads and standalone lethal writes, and reconstructs every in-window receipt/payload pair under the active danger root. Unit, strict-Clippy, and hosted-fixture construction gates are green locally; the prior exact-head PostgreSQL failure is diagnosed as a malformed Hall-location fixture and repaired with its required safe-arrival authority. Next obtain exact-head hosted proof, connect the server-owned trace aggregate and restart reconstruction, then promote the lethal tick only inside the durable death transaction and prove coordinator response-loss/restart behavior through PostgreSQL and real QUIC. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
