...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), and persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md) are complete. The active [`GB-M03-06A`](docs/tasks/GB-M03-06A.md)/[`GB-M03-02D`](docs/tasks/GB-M03-02D.md)/[`GB-M03-13`](docs/tasks/GB-M03-13.md) slice has the schema-`0042` atomic death/Echo graph, writer, immutable views, protocol `1.13`, and authenticated fail-closed dispatch. Schema `0043` now adds the normalized live `06B` evidence foundation: append-only clock and reward-qualified deed receipts plus a bounded ten-second trace graph that remains separate from the Bell checkpoint payload and must be removed by terminal death. The shared terminal arbiter gives death absolute same-tick precedence; the server death builder seals authenticated evidence; the execution adapter permits only that prepared lethal winner to call `PostgreSQL`; and a read-only recovery projection reconstructs the exact terminal receipt from the committed graph after process loss. Next implement the `0043` transactional repositories and server ingestion, then connect the internal private-life coordinator and prove committed death through hosted `PostgreSQL` and real QUIC with response loss/restart. Keep every player-visible death, successor, extraction/Recall, station admission, Core promotion, and normal route fail closed.
