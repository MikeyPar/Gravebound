...
The three-authority parent audit accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). Prerequisites through the shared per-transport writer are green at `2d59fa3` under hosted CI [`29625864479`](https://github.com/MikeyPar/Gravebound/actions/runs/29625864479). Commits `aa3c9ef`, `a63a36e`, and `f0db2c7` now close the audited implementation gaps with a terminal-first serializable PostgreSQL bootstrap, schema-63 durable extraction-intent acceptance/conflict audit, an opaque directory-owned `TerminalPending` permit, paired route/world content authority, and the live Caldus extraction actor/planner. Local formatting, strict workspace Clippy, workspace tests, guarded PostgreSQL-target compilation, content validation, and schema checks pass through `e58b37b`; run cumulative hosted PostgreSQL/restart/Windows/native proof next. Then compose the all-or-nothing `BoundCorePrivateLifeServer` and ordinary native route. Keep normal capability advertisement, Character Select `Play`, and Realm Gate interaction disabled until the complete runtime and evidence pass; keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed. The unregistered Sir Caldus [attack-motion](assets/core/bosses/sir_caldus/review/v2/README.md) and [idle/recovery](assets/core/bosses/sir_caldus/review/v3/README.md) review packs now cover four-frame idle, Shield Arc, Charge Lane, and recovery-to-guard strips with locked anchors/endpoints, 192/96 px readability, provenance, and standard/reduced camera mocks. They remain outside the asset registry and runtime content hashes pending one combined in-engine timing, attack-origin/hurtbox/collision, anchor-drift, and optimized native review.
