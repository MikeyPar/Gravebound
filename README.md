...
Commits through `09ae003` complete the server-owned `CorePrivateGameplayObservation` across microrealm, B1-B5, and Sir Caldus, publish validated latest-state QUIC datagrams at 15 Hz plus the final terminal frame, reset/rebind snapshot assembly by transport and actor generation, buffer datagrams behind matching reliable route authority, and render the accepted player/enemy/boss/projectile state in the native playfield. This follows the canonical GDD simulation/network rules, the Content Production Spec's closed Core route, and the Development Roadmap `GB-M03-03` gate. The Current Next Step is to expose the remaining ordinary Bell/door/B4/B3/B6 interactions and explicit Recall/extraction controls, then reconcile stored extraction, Recall, death-summary, Hall, and successor outcomes in the same native route. Telemetry, support lookup, hosting/platform evidence, and the user-deferred full audit/external gates follow.
