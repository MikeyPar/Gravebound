...
> **Project status:** M01 and M02 are closed. GB-M03 is approximately **72% complete**: identity, PostgreSQL persistence, Hall/private-world foundations, progression, the complete Core item/Vault lifecycle, Oath/Bargain, Ash, atomic death/Memorial/Echo persistence, and successful extraction/Emergency Recall/ResolutionHold are closed under hosted evidence. [`GB-M03-08`](docs/milestones/GB-M03-08-audit.md) passed at commit `2535e8c` on hosted CI [`29554811453`](https://github.com/MikeyPar/Gravebound/actions/runs/29554811453). Successor recovery has append-only protocol `1.17`, reliable kind `23`, `core_successor_v1`, pinned `1.16` compatibility, migration `0060`, death-side preset/reservation authority green through hosted CI [`29571991103`](https://github.com/MikeyPar/Gravebound/actions/runs/29571991103), the shared transaction-local `04D` starter seam, and a replay-first atomic successor writer. Its first hosted execution [`29579576357`](https://github.com/MikeyPar/Gravebound/actions/runs/29579576357) passed Linux and Windows gates and exposed one duplicate life-initializer call against migration `0033`; the idempotent repository-boundary repair and exact zero-state readback are locally green and await the replacement hosted PostgreSQL run. Authenticated server/QUIC and native handoff integration follow that proof. Normal route admission, telemetry, support, hosting/platform evidence, and final private-loop/cohort gates remain disabled.
...
Proceed with [`GB-M03-07`](docs/tasks/GB-M03-07.md) under accepted [`SPEC-CONFLICT-031`](docs/spec-conflicts/SPEC-CONFLICT-031-m03-successor-recovery-authority.md). Re-run the mandatory hosted `postgres_successor` gate with the migration-`0033`-compatible life initializer and exact zero-state readback. Once that proof is green, add the authenticated service and disposable real-QUIC route. Keep normal Character Select `Play`, production Realm Gate admission, Core promotion, and all M04+ features fail closed.
