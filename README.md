...
> **Project status:** M01 and M02 are closed. The current GB-M03 player route is assembled in optimized tester r22, while **14 of 23 fixed formal closure units are proven (61%)**. The stable denominator is 14 Roadmap work packages plus nine explicit M03 exit outcomes; partial gates do not count. The open engineering work is final ordinary-route/25-journey proof, committed-source coverage for four telemetry families, and hosted support proof. The private cohort, hosting/backup rehearsal, and Steamworks evidence remain external gates. See the [formal gate audit](docs/milestones/GB-M03-gate-audit.md) and [r22 release manifest](docs/evidence/GB-M03-tester-r22-manifest.md).
