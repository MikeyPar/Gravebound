...
[`GB-M03-04G`](docs/milestones/GB-M03-04G-audit.md), parent [`GB-M03-04`](docs/milestones/GB-M03-04-audit.md), persistence slice [`GB-M03-02C`](docs/milestones/GB-M03-02C-audit.md), and native presentation slice [`GB-M03-06D`](docs/milestones/GB-M03-06D-audit.md) are complete. Under the canonical GDD, Content Production Spec, and Development Roadmap, commits `fb8f56a` through `e16424e` define, replay, restart, adversely validate, and enforce exact causal-outbox cardinality for the versioned death/destruction/Memorial/Echo terminal signature. Commits `7331a13`, `eab019c`, and `f50bbd9` add measured database/runtime residue, exact connection-task teardown, and ten authenticated real-QUIC/PostgreSQL death-performance journeys. Hosted CI `29488569454` accepted the checked-in [`GB-M03-06E` latency report](docs/performance/GB-M03-06E-death-latency.json): durable acknowledgement to interactive native summary measured 7.454 ms median, 12.868 ms p95/max, every sample passed the two-second target, exact replay preserved the canonical signature, and transport/task/session/transaction/lock residue was zero. Commit `f3e44cd` adds the explicit release-profile 30-minute real-QUIC/PostgreSQL persistence soak with exact replay, canonical-signature, reconnect, combined-process memory, and teardown gates; dedicated workflow `29489909161` is running it. Commits `2d61ab5` and `e32cf1f` persist ordinary/incident/administrative provenance and make the existing deferred graph validator require ordinary provenance in the exact `ECH-001` predicate. Commit `da62c66` parameterizes the production-authority death fixture across level, clocks, deeds, provenance, Echo decisions, identities, and sequential mortal lives so the matrix can prove an existing Available Echo without manufacturing Echo rows. The active [`GB-M03-06E`](docs/tasks/GB-M03-06E.md) step is to archive the soak report, execute the hosted eligibility/promotion matrix, and publish the joint `06`/`02D`/`13` audits. Keep player-visible death, successor, extraction/Recall, station admission, Core promotion, and the normal route fail closed until their integrated owners pass.
