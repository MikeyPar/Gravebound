...
Source `525b423` remains packaged as [optimized tester r32](docs/evidence/GB-M03-tester-r32-manifest.md) with both executables, accessibility controls, protocol 1.25, and migrations through schema 78. Hosted run [`30050712798`](https://github.com/MikeyPar/Gravebound/actions/runs/30050712798) passed the focused schema-79 PostgreSQL proof, including legitimate live damage before the first optional TECH-023 checkpoint, then reached committed Bell B0 route authority and exposed a real presentation boundary: the fixed-dungeon handoff retained the micro-realm position and published no B0 gameplay snapshot. Commit `79668fe` now relocates the retained participant to the compiled `room.bell.vestibule_01` safe-entry anchor `(3,5.5)`, publishes a forced same-tick noncombat snapshot without advancing the combat clock, and makes the journey require exact route version, tick, spawn, and zero hostiles. Focused simulation, driver, snapshot, strict server, and journey-target gates pass. The exact-25 evidence compiler exists at commit `886387f`; it does not claim 25 completed private loops yet. These changes follow `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`. The Current Next Step is active hosted route run [`30052832985`](https://github.com/MikeyPar/Gravebound/actions/runs/30052832985), followed by a clean schema-79 tester release, 25 complete ordinary loops, current aggregate timing, optimized Realm Gate capture, and bounded operational telemetry proof. Private-cohort comprehension, backup/restore, hosting, destination/privacy approval, and Steamworks evidence remain required. Formal progress remains **15/23 (65%)** until a whole package or exit outcome closes.
