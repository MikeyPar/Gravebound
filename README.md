...
> **Project status:** M01 and M02 are closed under their recorded gates. M03 has closed identity, PostgreSQL foundations, exact world-flow content, atomic dormant transfers, Hall/private-microrealm simulation and native evidence, progression through durable reward placement, the complete first Oath/Bargain package, and the minimal Ash wallet under their recorded audits. `GB-M03-03D` now compiles the exact six-normal/two-miniboss behavior roster into strict hashed records and renderer-independent 30 Hz definitions with trace-proven scheduler maxima, deterministic COM-006 routes, deterministic room rotation/navigation, the branch-disabled B0-to-B6 corridor chain, golden-traced schedulers for all five Core-authored kits, and the authoritative `pack.bell.01` warning/combat/clear/reset lifecycle. Parent `GB-M03-02`, `03`, and `04` remain open alongside permanent death, successor recovery, extraction/Recall, telemetry, support, Echo, platform, and final private-loop gates.
...
[`GB-M03-03E`](docs/milestones/GB-M03-03E-audit.md) is complete under its three-authority audit. Strict Caldus content, exact 180-second combat, personal reward terminality, committed extraction, idempotent `HallDefault` transfer, mandatory PostgreSQL coverage, and the 32-frame native evidence matrix pass without importing `GB-M03-08` inventory conversion or opening normal ingress. [`GB-M03-03F`](docs/tasks/GB-M03-03F.md) is active under approved [`SPEC-CONFLICT-025`](docs/spec-conflicts/SPEC-CONFLICT-025-m03-native-transition-and-reconnect-ux.md). Its strict 29-key transition/error copy, sequence-safe projection, native loading/error/LinkLost/reconnect surface, and optimized 33-frame standard/reduced-effects evidence matrix now pass focused tests, warnings-denied Clippy, and original-resolution inspection. The guarded real-QUIC baseline composes dormant atomic entry with committed Caldus Hall return, response-loss replay, and content-mismatch rejection; its hosted PostgreSQL rerun is the remaining baseline gate. Complete the integrated adverse-case matrix, then record exact route/reconnect timing, frame-time p95/p99, memory, 25 scripted journeys, and the 30-minute optimized soak. Continue the parent roadmap audit, `GB-M03-04E`-`04G`, and the approved `SPEC-CONFLICT-009` death foundation as dependencies become ready. Keep GB-M03-08 inventory extraction/Overflow/ResolutionHold, 50-Ash purge, paid Oath change, Core promotion, Character Select `Play`, Realm Gate admission, BB1/BS1 traversal, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
