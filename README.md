...
The three-authority parent audit identified the missing normal composition root and accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). [`GB-M03-03G`](docs/tasks/GB-M03-03G.md) slice 1 is green at exact source `26f9c66` under hosted CI [`29616169514`](https://github.com/MikeyPar/Gravebound/actions/runs/29616169514): production read/mutation routing, restart-stable authority-scoped IDs, exact Hall/danger entry, cancellation-safe two-phase generation/version-bound Bell leases outside PostgreSQL locks, same-root fixed-dungeon commit, typed replay/abort/reconcile, and fail-closed later destinations passed Linux tests/lint, PostgreSQL, optimized Windows construction, and optimized native evidence. Implement append-only protocol `1.18` `CorePrivateRouteStateV1`, full composed-route content binding, room-aware client readiness, and the capacity-one actor next. Keep normal capability advertisement, Character Select `Play`, and Realm Gate interaction disabled until the live actor, terminal owners, ordinary native route, and cumulative evidence are attached; keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed.
