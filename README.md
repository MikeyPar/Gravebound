...
[`GB-M03-04E`](docs/milestones/GB-M03-04E-audit.md) is complete under its three-authority audit with deterministic four-slot swaps, schema-27 replay-safe PostgreSQL persistence, bounded projection, authoritative four-slot combat construction, approved absolute-health rebuild, native comparison UI, and the eight-case 18-icon visual matrix. [`GB-M03-04F`](docs/tasks/GB-M03-04F.md) is active: schema-28 CharacterSafe/Vault custody, the exact 8/160/8 planner, and the normalized schema-29/30 serializable transfer repository pass hosted PostgreSQL. The bounded dormant coordinator and all-or-nothing danger-entry auto-deposit are implemented: the caller names no destination, successful preflight captures post-mutation aggregate versions, and capacity rejection precedes identity allocation. Require the expanded hosted coordinator/world-flow matrix, then run the full gates and publish the `04F` audit. `04G` retains real-QUIC item/Vault lifecycle and parent closure. Keep death/extraction/Recall, Overflow/ResolutionHold, salvage/crafting, Core promotion, Character Select `Play`, Realm Gate admission, seeded selection, the normal player route, and affected Hall stations fail closed until their owning gates pass.
