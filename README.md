...
The three-authority parent audit identified the missing normal composition root and accepted [`ADR-037`](docs/decisions/ADR-037-normal-core-private-route-composition.md) under the route contract already approved by [`SPEC-CONFLICT-006`](docs/spec-conflicts/SPEC-CONFLICT-006-m03-world-flow-contract.md). [`GB-M03-03G`](docs/tasks/GB-M03-03G.md) slice 1 is green at exact source `26f9c66` under hosted CI [`29616169514`](https://github.com/MikeyPar/Gravebound/actions/runs/29616169514), and the append-only protocol `1.18` plus room-aware native projection checkpoint is green at exact source `342889b` under hosted CI [`29619764945`](https://github.com/MikeyPar/Gravebound/actions/runs/29619764945). The exact Core private-life content aggregate, capacity-one route actor, and restart-stable PostgreSQL generation allocator are implemented at `07c5cc8`/`dce9a27`/`bc5c59d`, covering the closed Hall/micro-realm/B0-B6/Caldus graph, bounded single-writer ownership, RAII Bell reservation, terminal generation retirement, and monotonic audited schema-62 allocation. Run their cumulative hosted Linux/PostgreSQL/Windows proof next; after green, compose the dedicated `BoundCorePrivateLifeServer`, shared reliable sequencing, live terminal owners, terminal-first crash restore, and ordinary native route. Keep normal capability advertisement, Character Select `Play`, and Realm Gate interaction disabled until the complete runtime and evidence pass; keep Core promotion, M04+ content, telemetry, support, hosting/platform surfaces, and unrelated Hall stations fail closed.
