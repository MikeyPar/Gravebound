...
4. Start `Start All Clients.cmd` once. It launches four distinct credentials quickly enough to form one four-player arena. Use numbered client launchers only for the reconnect step.
...
4. Avoids hostile projectiles, contributes to defeating the hostile set, and presses `E` near an eligible personal pickup.
...
Reconnect check: close one client during combat, then relaunch the same numbered client before three seconds elapse. The same character must reattach; it remains vulnerable while LinkLost. If it does not reconnect in time, only that player automatically Recalls and survivors continue.
