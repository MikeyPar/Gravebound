...
6. Major-version mismatch is incompatible. A server accepts an equal-or-older client minor version within the same major only when the exact required build and content manifest match; a newer client minor is rejected as `ProtocolUnsupported`. Required build mismatch is `UpdateRequired`. Protocol evolution must add golden fixtures before changing the pinned canonical bytes.
