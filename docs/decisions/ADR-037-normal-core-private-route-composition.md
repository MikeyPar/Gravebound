...
**Implementation checkpoint (2026-07-17):** exact source `e069987` is green under hosted CI [`29633952947`](https://github.com/MikeyPar/Gravebound/actions/runs/29633952947). It closes the dormant persistent foundation and per-account shared-writer session/Recall owner under PostgreSQL, strict Linux, Windows release, and optimized native evidence. Commit `cf933f2` adds the dynamic extraction runtime: only an opaque coordinated executor proof can publish a fully validated result; command-time generation ticks, lineage/restore-bound competing-terminal retirement, cancellation-safe single delivery, replay-correct handoff, exact Hall installation acknowledgement, and aggregated route/runtime cleanup are enforced. Commit `a30e644` adds the terminal-first process bootstrap adapter and post-commit/replayed Character Select, Hall, and microrealm reconciliation. Corrective commit `51be807` keeps extraction/Recall restart states terminal-first by withholding Hall actors until exact stored-result delivery acknowledgement; extraction request sequence remains transport metadata supplied by the exact retry rather than invented recovery state. First-time identity, storage blocking, restart crash restore, committed terminal precedence, durable actor generations, exact replay, changed-material rejection, and shutdown retirement are explicit; focused tests, the complete 293-test server suite, all server targets, and strict server all-target Clippy pass locally. Shared-writer terminal publication, the complete live actor, and the dedicated ordinary root remain open, so capability advertisement remains false.
