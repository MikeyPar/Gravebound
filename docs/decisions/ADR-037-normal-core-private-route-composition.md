...
**Implementation checkpoint (2026-07-17):** exact source `ce0373c` is green under hosted CI [`29631897041`](https://github.com/MikeyPar/Gravebound/actions/runs/29631897041). It closes the dormant persistent foundation and per-account shared-writer session/Recall owner under PostgreSQL, strict Linux, Windows release, and optimized native evidence. Commit `cf933f2` adds the locally green dynamic extraction runtime: only an opaque coordinated executor proof can publish a fully validated result; command-time generation ticks, lineage/restore-bound competing-terminal retirement, cancellation-safe single delivery, replay-correct handoff, exact Hall installation acknowledgement, and aggregated route/runtime cleanup are enforced. Its 9/9 intent-runtime tests, 11/11 execution tests, complete 286-test server suite, workspace all-target tests, strict workspace Clippy, and independent authority re-audit pass. Transition/bootstrap reconciliation, session binding, the complete live actor, and the dedicated ordinary root remain open, so capability advertisement remains false.
