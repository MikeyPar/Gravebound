...
**Implementation checkpoint (2026-07-17):** exact source `037efe6` is green under hosted CI [`29630076008`](https://github.com/MikeyPar/Gravebound/actions/runs/29630076008). It closes the shared per-transport writer, terminal-first serializable bootstrap, schema-63 durable extraction-intent acceptance/conflict audit, route-owned `TerminalPending` permit, paired route/world content authority, live Caldus intent actor/planner, response-loss/reconnect replay, PostgreSQL transaction/restart coverage, strict Linux checks, Windows release construction, and optimized native evidence. Commit `26c420b` adds the dormant persistent foundation, constructing exact route content and reusable identity, world-flow, progression, death, Oath/Bargain, storage, successor, terminal-execution, and route-directory owners before socket binding. Commit `551c527` adds the per-account session owner: one monotonic transport generation and one shared reliable writer exist from handshake onward; danger-only Recall binds dynamically, rebinds before transport handoff visibility, preserves exact LinkLost timing, rejects stale teardown, and retires without closing the continuing Hall writer. Its focused and complete server suites and strict all-target Clippy pass locally. Dynamic extraction, transition/bootstrap reconciliation, the complete live actor, and the dedicated ordinary root remain open, so capability advertisement remains false.
