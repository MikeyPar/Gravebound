...
- Schema-0070 and schema-0071 sources carry immutable per-row origin context. Terminal-family attribution originally used a temporary caller-supplied context, but no adapter may reconstruct missing session facts from live state. Raw account IDs are consumed only inside the adapter to produce keyed pseudonyms and never cross into an event envelope.
- Additive schema 0073 completes that terminal boundary for newly committed death, extraction, Emergency Recall, and successor outbox rows. A bounded, nonlocking lookup binds the one durable telemetry-session interval covering the owning terminal transaction. Session boundaries and terminal occurrence times are all PostgreSQL transaction-clock facts; application-authored session observation timestamps cannot relabel a terminal. Missing or overlapping context stores no origin and cannot reject gameplay. Pre-0073 terminal history is not backfilled. The terminal adapter accepts only the separated pseudonymization key and joins the immutable origin session for all TEL-001 attribution; runtime-supplied live context is removed.
- Schema 0073 also derives exact death session duration from the captured PostgreSQL session-start boundary and exact item-power band from the immutable death summary/destruction/item graph. The authoritative death graph does not yet persist boss phase, party size/contribution, or detailed ping/jitter/loss/correction inputs. Those required `TEL-003` fields remain explicitly unavailable rather than fabricated; the atomic death contract must add them before `GB-M03-09` can close.
...
Production-blocking verification covers structural redaction, rejection of sensitive-looking labels and zero crash signatures, disabled operation, offline bounded capacity, duplicate polling, backpressure, exact post-delivery acknowledgement, exporter-response failure, and restart-style re-poll. Schema-0070 adds compile-checked disposable-PostgreSQL coverage for atomic onboarding projection, logical-session replay and canonical transitions, typed crash observations, immutable per-row context, restart recovery, and one-way publication. Schemas 0071/0072 add focused coverage for canonical item-ledger projection, exact reward/starter origin, replay/conflict, immutable payloads, no-session gameplay availability, restart re-poll, bounded reads, exact one-way acknowledgement, and the hosted integer-literal compatibility defect. Schema 0073 adds compile-checked repository journeys for real death, extraction, Recall, and successor commits; no-session gameplay availability; PostgreSQL-clock skew/ambiguity/handoff behavior; immutable origin guards; adapter-loss re-poll; exact acknowledgement; and one-way publication. Focused runtime tests additionally cover generation-safe handoff/reconnect, orphan replacement, clean exit, truthful transition failure, accepted-client platform attribution, cohort isolation, explicit region/environment configuration, and failure containment. Hosted schema-71/72/73 execution, completion of all `TEL-003` death inputs, enabled adapter/exporter wiring, and full statistical/load/collector/retention audits remain intentionally open until their M03 evidence exists; disabled production-worker ownership is implemented without enabling export.

This boundary gives M03 a typed and recoverable telemetry seam without making analytics part of gameplay availability. Export remains disabled by default. Complete `TEL-003` source facts, an approved destination, enabled adapter/exporter wiring through the existing worker owner, destination-specific retention/deletion review, and hosted operational evidence remain necessary before `GB-M03-09` can close.
