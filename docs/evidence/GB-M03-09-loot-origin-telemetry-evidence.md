...
The PostgreSQL journey remains ignored by default. Hosted run [`29907197863`](https://github.com/MikeyPar/Gravebound/actions/runs/29907197863) applied schemas 71/72 and passed both focused journeys at source `047c9f3`: `item_writes_succeed_without_a_telemetry_session_or_loot_sidecar` and `telemetry_sources_are_transactional_replay_safe_restart_safe_and_one_way` completed `2 / 2` in 1.29 seconds. This proves the additive overload repairs the exact projection defect while preserving no-session gameplay availability, replay, restart, and one-way publication. The overall workflow later failed in the independent production-route test during Character Select; that failure does not invalidate the earlier completed PostgreSQL loot-origin results and is not represented as a green aggregate run. Remote export remains disabled; no destination, processor, or retention claim is made here.

## Current Next Step

Following `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`, schemas 71/72 and both disposable-PostgreSQL loot-origin journeys are now hosted-proven at source `047c9f3`. The Current Next Step is exact terminal-family origin binding plus the remaining atomic `TEL-003` death facts, followed by bounded lag/restart proof through the already-owned disabled production worker. Do not enable remote export until the `ADR-039` destination, region, access, encryption, retention, deletion, backup-expiry, and privacy-notice review passes.
