...
Obtain a green mandatory hosted PostgreSQL run for exact source `58e417e`, which repairs the canonical level-4 XP and inactivity-ineligibility fixtures exposed by the preceding CI run. Then execute the production extraction outage, process-restart, response-loss, and competing-terminal matrix against the coordinator/session composition. Do not bind normal-server construction or enable admission until that matrix proves identical durable authority, single activation, exact replay, terminal release, and zero residue. Once green, bind only the owned Core gates and begin the 25 full-loop journeys plus optimized native evidence.
