...
Obtain a green mandatory hosted PostgreSQL run for exact source `6d1c0eb`. The preceding run proves the `58e417e` B3 repair green and Windows release construction green, then exposes two stale Caldus fixture boundaries now corrected by unique authority IDs and an active lineage seed. After hosted green, compose the live death, five-producer terminal, production tick-source, danger-activation, and dispatch owners behind an all-or-nothing authority proof. Do not advertise or bind normal admission until those owners and their shutdown order are proven together.
