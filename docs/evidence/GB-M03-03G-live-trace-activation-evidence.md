...
Implement the mandatory factory's receiver loop. For each ordered frame, capture exact session network, Recall, and status authority; register the immutable frame identities; persist/replay nonlethal trace ticks; run lifetime and permadeath-combat clocks independently across route controls and waits; and hold lethal acknowledgement until the durable death transaction returns the exact stored receipt. Then add extraction, Recall, disconnect-recovery, and server-fault producers to the same arbiter.
