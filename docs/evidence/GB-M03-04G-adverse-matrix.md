...
| Stale/foreign authority | `safe_inventory::caller_can_name_only_source_and_versions`; `postgres_world_flow::stale_foreign_and_corrupt_state_fail_closed_without_danger_allocation`; repository binding tests | authenticated account, selected living Hall character, source custody, and aggregate versions bind every mutation | Cumulative rerun pending |
