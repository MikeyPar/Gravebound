...
The [bounded, lossless, acknowledged terminal-frame feed](GB-M03-03G-terminal-frame-feed-evidence.md) is now implemented for every bound frame, including empty-damage frames. Next total-order the Bell/room/reward/exit control events between frames, bind the exact danger-entry restore authority, and replace the disabled terminal-ownerless session path with lifetime/deeds/custody authority and all five terminal producers behind the all-or-nothing private-life builder. Keep normal admission disabled until that owner graph, durable death path, restart behavior, and shutdown order pass together.
