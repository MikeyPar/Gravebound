...
The [bounded, lossless, acknowledged terminal event stream](GB-M03-03G-terminal-frame-feed-evidence.md) now covers every bound frame plus exact Bell, B0-B6, B3, B4, and Caldus controls. Next bind the exact danger-entry restore authority and replace the disabled terminal-ownerless session path with independent lifetime/permadeath clocks, deeds, custody, entity identities, per-tick providers, and all five terminal producers behind the all-or-nothing private-life builder. Keep normal admission disabled until that owner graph, durable death path, restart behavior, and shutdown order pass together.
