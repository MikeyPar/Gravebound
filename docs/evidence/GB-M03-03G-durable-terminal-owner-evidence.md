...
Commits through `87debaf` place this owner behind the terminal-first normal server/client and continuously publish committed driver route changes through the winning writer. Actor generation/state-version deduplication spans connection and reward producers, while reward-pending observations cannot overtake ordered progression or pending-inventory delivery. Next add complete runtime-owned gameplay observations, 15 Hz latest-state datagrams, and client buffering behind matching reliable route authority; then complete interaction and stored terminal destination reconciliation before the deferred hosted PostgreSQL/QUIC, restart, response-loss, shutdown, 25-journey, performance, and visual audit.
