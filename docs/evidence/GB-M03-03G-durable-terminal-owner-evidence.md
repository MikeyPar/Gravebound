...
Commits through `c2d18ba` place this owner behind the complete normal extraction and lethal-death routes. Lethal authority reaches the durable summary barrier and production successor UI; a stored successor retires the exact completed danger task, refreshes the roster, and returns via ordinary Character Select -> Hall authority. Migration `0065` aligns locked-item and deferred-SQL Echo power for Black/Unique gear and uses only the approved Core silhouette. This remains governed by the canonical GDD, Content Production Spec, and Development Roadmap. Next expose the read-only Memorial Wall through Hall authority before telemetry/support/hosting packaging and the user-deferred hosted PostgreSQL/QUIC, restart, response-loss, shutdown, 25-journey, performance, visual, and external audit.
