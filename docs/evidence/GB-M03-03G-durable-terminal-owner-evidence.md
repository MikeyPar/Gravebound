...
Commits through `acc721f` place this owner behind the complete normal extraction, lethal-death, Memorial, and successor routes. Both the terminal summary and Hall use the bounded newest-first read-only Memorial model and exact stored snapshot selection without localized reconstruction. This remains governed by the canonical GDD, Content Production Spec, and Development Roadmap. Next finish telemetry/support/hosting packaging before the user-deferred hosted PostgreSQL/QUIC, restart, response-loss, shutdown, 25-journey, performance, visual, and external audit.
