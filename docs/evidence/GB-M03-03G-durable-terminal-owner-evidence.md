...
Commits through `5b69b5b` place this owner behind the terminal-first normal server/client, complete ordinary Bell/door/B4/Recall/extraction dispatch, and bind extraction to server-issued identity plus coherent pending-inventory authority. Exact mutation retry is retained client-side; stored-result delivery is required before exact Hall installation and replay-authority consumption. This remains governed by the canonical GDD, Content Production Spec, and Development Roadmap. Next complete durable lethal-death summary, Memorial, Hall, and successor reconciliation before telemetry/support/hosting packaging and the user-deferred hosted PostgreSQL/QUIC, restart, response-loss, shutdown, 25-journey, performance, visual, and external audit.
