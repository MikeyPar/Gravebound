...
Commits through `ca0953d` place this owner behind the terminal-first `BoundCorePrivateLifeServer` and negotiated native route. The winning generation owns one reliable writer; durable dispatch completes before acknowledgement; Hall and danger datagrams remain authority-bound; route events are state-version deduplicated; and native control requires matching capability, location, route revision, and compiled readiness. Next publish live driver observations and bounded snapshots, complete interaction dispatch and stored extraction/Recall/death destination reconciliation, then run the deferred hosted PostgreSQL/QUIC, restart, response-loss, shutdown, 25-journey, performance, and visual audit.
