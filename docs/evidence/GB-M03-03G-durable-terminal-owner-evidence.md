...
Commits through `09ae003` place this owner behind the terminal-first normal server/client, project complete committed player/enemy/boss/projectile observations, publish bounded 15 Hz plus final-terminal datagrams, and reconcile them by transport epoch, actor generation, route state version, and generation-derived player identity before native rendering. This remains governed by the canonical GDD, Content Production Spec, and Development Roadmap. Next complete ordinary Bell/door/B4/B3/B6 interaction dispatch, explicit Recall/extraction controls, and stored terminal destination/summary/successor reconciliation before the deferred hosted PostgreSQL/QUIC, restart, response-loss, shutdown, 25-journey, performance, and visual audit.
