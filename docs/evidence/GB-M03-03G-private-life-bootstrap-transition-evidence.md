...
Commit `93dde0a` binds dynamic extraction, Recall, and restart recovery through the private-life session owner and its single `CoreReliableWriter`. The real-QUIC composition proof uses one exact writer identity, contiguous sequences `1/2/3`, coordinated reconnect, harmless stale detach, session-owned retirement, and zero-residue shutdown. Next, compose live movement, combat, rewards, pending inventory, and all five terminal producers through that persistent session, then close ordinary QUIC admission, restart/response-loss coverage, 25 full journeys, and optimized native visual evidence before enabling Character Select `Play`, the production Realm Gate, or normal extraction/Recall capability flags.
