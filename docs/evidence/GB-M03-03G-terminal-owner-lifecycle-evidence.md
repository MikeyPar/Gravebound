...
Implement the production terminal-owner factory over the existing live damage trace and durable death services. Add immutable simulation-to-journal entity registration, independent lifetime and permadeath-combat clocks, deeds, custody, per-tick network/Recall/status providers, and lethal death, extraction, Recall, disconnect-recovery, and server-fault producers. Keep normal admission disabled until this real owner replaces the intentionally absent production factory and proves complete three-scene shutdown/restart behavior.
