...
Commit `8cd626e` installs the sealed planner in this mandatory production owner and holds every frame behind exact clock/trace/deed/death durability. Commit `50d858a` binds actor-owned Recall movement, action blocking, and trace phase to the same simulation boundary. Commits `5d9dc33` and `1ec0285` supply the coherent current-terminal snapshot and route exact explicit/LinkLost Recall plus disconnect recovery through that same owner. Next compose prepared extraction, exit-ready heartbeats, and verified-fault restoration. Keep normal admission disabled until the five-way coordinator and focused three-scene shutdown/restart behavior pass.
