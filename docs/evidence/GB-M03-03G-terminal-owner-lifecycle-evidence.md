...
Commit `8107752` adds [transaction-validated live-trace activation and immutable simulation-to-journal identity](GB-M03-03G-live-trace-activation-evidence.md), and commit `e869f01` adds the [same-boundary terminal tick context](GB-M03-03G-terminal-tick-context-evidence.md). Network transitions are now lossless; Core status authority is explicitly empty; Recall remains intentionally inactive until its server-only actor proxy is wired. Next build the PostgreSQL death-context planner, connect clocks/trace and the Recall proxy in the production factory, and hold lethal acknowledgement for the exact stored receipt. Keep normal admission disabled until all five terminal producers and three-scene shutdown/restart behavior pass together.
