...
The damage dependency and its [bounded, lossless terminal event stream](GB-M03-03G-terminal-frame-feed-evidence.md) are implemented locally through exact Bell, B0-B6, B3, B4, and Caldus authority. Controls retain the current simulation tick and do not advance the independent lifetime/permadeath clocks. Next bind the exact danger-entry restore authority and replace the disabled terminal-ownerless session path with trace, clocks, deeds, custody, entity identities, per-tick providers, and all five terminal producers inside the all-or-nothing private-life authority builder. Normal capability advertisement remains disabled until the complete owner graph and shutdown order pass together.
