...
This slice proves the source path through the complete fixed dungeon, durable ordinary extraction, ordinary lethal death, durable summary/Memorial/trace reads, successor creation, and checked Hall/danger control. Hosted run [`30028182217`](https://github.com/MikeyPar/Gravebound/actions/runs/30028182217) exercised `2,698` live public microrealm snapshots through tick `5,396`, including all eight authored hostile identities and projectile combat, then isolated defeated-snapshot targeting. Commit `2e8d2f4` restricts selection to living authority and passes a focused authored-pack clear. Run [`30047477525`](https://github.com/MikeyPar/Gravebound/actions/runs/30047477525) received the exact committed entry route and matching public snapshot, but the journey then waited for a second observation before sending any activation input and reached the QUIC idle deadline. Commit `1ad117e` immediately sends ordinary eastward movement from the already-verified entry snapshot and preserves the public-input-only boundary. Run [`30049060896`](https://github.com/MikeyPar/Gravebound/actions/runs/30049060896) reached the first ordinary damage evidence before TECH-023's optional periodic checkpoint, where PostgreSQL rejected schema 43's contradictory trace-tick-to-checkpoint foreign key. Forward-only schema 79 drops only that redundant key; the immutable danger-entry restore FK, retained ingest receipt, content hashes, exact locked checkpoint validation, trace graph, and terminal promotion remain intact. Optimized tester r32 remains the latest packaged runtime at source `525b423`, protocol 1.25, and schema 78. Under `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`, the Current Next Step is focused hosted schema-79/pre-checkpoint proof followed by the complete two-terminal path, then the 25-complete-loop matrix, current aggregate timing, and optimized Realm Gate capture. No `GB-M03-03` or M03 closure credit is awarded until those whole gates pass.
