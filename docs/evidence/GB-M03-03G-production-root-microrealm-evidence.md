...
This slice proves the source path through the complete fixed dungeon, durable ordinary extraction, ordinary lethal death, durable summary/Memorial/trace reads, successor creation, and checked Hall/danger control. Hosted run [`30053995319`](https://github.com/MikeyPar/Gravebound/actions/runs/30053995319) isolated the B1 terminal-owner failure after the exact B0 snapshot, and run [`30066885083`](https://github.com/MikeyPar/Gravebound/actions/runs/30066885083) named it as `CorruptStoredPrivateLifeBootstrap`. Commit `eb0c6b4` repairs that contract: the durable same-lineage Bell transfer atomically advances the open lineage's current content ID to `dungeon.bell_sepulcher` while preserving the exact restore point and immutable transfer receipt; mismatch fails closed. Focused PostgreSQL bootstrap/restart/replay proof passes. Hosted run [`30069213403`](https://github.com/MikeyPar/Gravebound/actions/runs/30069213403) passed schema-79/pre-checkpoint proof and reached route tick 1300 before the evidence driver reused primary-press identity `1`; harness correction `87a7967` is under corrected hosted run [`30071229521`](https://github.com/MikeyPar/Gravebound/actions/runs/30071229521). [Optimized tester r38](GB-M03-tester-r38-manifest.md) bundles PostgreSQL 17.10 and proves persistent server plus native client startup without Docker or either reported Bevy query panic. Under `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`, the Current Next Step is hosted green single-route completion, the 25-complete-loop matrix, current aggregate timing, and optimized Realm Gate capture. No `GB-M03-03` or M03 closure credit is awarded until those whole gates pass.
