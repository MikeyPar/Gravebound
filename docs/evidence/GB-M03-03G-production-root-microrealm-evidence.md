...
This slice proves the source path through the complete fixed dungeon, durable ordinary extraction, ordinary lethal death, durable summary/Memorial/trace reads, successor creation, and checked Hall/danger control. It does not yet prove hosted execution of either terminal branch, reconnect/restart repetition, the 25-journey matrix, current aggregate timing, or optimized Realm Gate capture. The Current Next Step is hosted disposable-PostgreSQL/real-QUIC execution of this exact two-terminal path, followed by the 25-journey matrix. No `GB-M03-03` or M03 closure credit is awarded here.
