...
This slice proves the source path through the complete fixed dungeon, durable ordinary extraction, ordinary lethal death, durable summary/Memorial/trace reads, successor creation, and checked Hall/danger control. Hosted run [`30028182217`](https://github.com/MikeyPar/Gravebound/actions/runs/30028182217) proved staged-to-active Realm Gate lineage activation and exercised `2,698` live public microrealm snapshots through tick `5,396`, including all eight authored hostile identities and projectile combat. Two hostiles reached zero health, but their presentation-retained snapshots remained nearest to the player; the ordinary driver did not filter by health and repeatedly fired at those defeated targets until its bounded timeout. Commit `2e8d2f4` restricts target selection and timeout diagnostics to living authoritative health, while a focused runtime regression clears the same authored pack using only public snapshot inputs and keeps the player alive. Optimized tester r31 remains the latest runtime package because this correction changes test authority rather than the game executable. Under `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`, the Current Next Step is an exact-source hosted rerun for this two-terminal path, followed by the 25-complete-loop matrix, current aggregate timing, and optimized Realm Gate capture. No `GB-M03-03` or M03 closure credit is awarded until those whole gates pass.
