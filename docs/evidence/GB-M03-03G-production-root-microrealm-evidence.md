...
This slice proves the source path through the complete fixed dungeon, durable ordinary extraction, ordinary lethal death, durable summary/Memorial/trace reads, successor creation, and checked Hall/danger control. Hosted run [`30022764927`](https://github.com/MikeyPar/Gravebound/actions/runs/30022764927) proved exact staged-to-active Realm Gate lineage activation and returned the accepted durable danger transfer after every preceding mandatory PostgreSQL suite passed. It then timed out waiting for a simulation-backed route frame because the control-plane publisher emitted the new route at tick zero and the shared reliable deduplicator suppressed the identical first-frame projection. Commit `c4f44e7` derives a zero-tick danger publication from the already committed driver observation while preserving Hall tick-zero publication and monotonic route versions. Optimized tester r31 packages that exact source after every production-blocking package gate passed. Under `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`, the Current Next Step is exact-source hosted run [`30024816019`](https://github.com/MikeyPar/Gravebound/actions/runs/30024816019) for this two-terminal path, followed by the 25-complete-loop matrix, current aggregate timing, and optimized Realm Gate capture. No `GB-M03-03` or M03 closure credit is awarded until those whole gates pass.
