...
| Unpromoted Core death-presentation validation | `cargo run --locked -p tools_content -- validate-core-death-view` |
...
- **M03 Tester Build:** `.\tools\dev.cmd m03-package` compiles and validates the optimized native client, exercises every packaged launch mode, publishes one revision-stamped tester folder and matching ZIP, and removes older M03 tester packages from `dist`.
