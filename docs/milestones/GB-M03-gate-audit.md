# GB-M03 Formal Gate Audit

**Status:** IN PROGRESS — `15 / 23` fixed closure units are proven (**65%**).

## Design authorities

This audit reads all three authorities together:

1. `Gravebound_Production_GDD_v1_Canonical.md`
2. `Gravebound_Content_Production_Spec_v1.md`
3. `Gravebound_Development_Roadmap_v1.md`

The percentage uses a stable denominator: the Roadmap's 14 named `GB-M03` work packages plus nine explicit M03 exit-gate outcomes. A unit counts only when its implementation, required automated evidence, and applicable human or operational evidence are complete. Partial credit is never rounded into a pass.

## Work-package closure (`11 / 14`)

| Package | Verdict | Evidence boundary |
| --- | --- | --- |
| `01`, `02`, `04`, `05`, `06`, `07`, `08`, `10`, `11`, `12`, `13` | Proven | Published task evidence and three-authority package audits close their owned contracts. |
| `03` | Open | The assembled ordinary route is packaged and its production-root source reaches the stable post-Caldus exit, but terminal branches and final no-command PostgreSQL/real-QUIC 25-journey evidence are not yet published. |
| `09` | Open | Terminal/onboarding/session/crash telemetry is committed-source backed; hosted schema-77 proof covers loot/terminal origins and run `30048509525` proves schema-78 atomic `TEL-003` upgrade/restart/fresh projection. Bounded operational exporter integration and destination/privacy/accuracy proof remain open. |
| `14` | Open | Provider-pinned hosting, backup/restore, rollback, and owner-supplied Steamworks evidence remain external gates. |

## Exit-gate closure (`4 / 9`)

| Exit outcome | Verdict | Evidence boundary |
| --- | --- | --- |
| Restart preservation | Proven | Hosted component evidence covers inventory, Vault, death, Memorial, Echo, extraction, Recall, and successor state. Final route-level repetition remains useful corroboration, not a reason to erase the component pass. |
| No duplicate durable mutation | Proven | Transactional replay/restart evidence covers item, currency, character, death, Echo, and terminal results. |
| Atomic qualifying death | Proven | Death, destruction, Memorial, and Echo commit together or not at all. |
| Median death-to-successor control `<15 s` | Proven | The authenticated PostgreSQL/QUIC 25-journey successor report records a median far below the limit. |
| Ordinary no-command route plus 25 full loops | Open | Earlier 25-journey reports are valuable subsystem evidence, not the final production-server route sweep. |
| Median login-to-control `<30 s` | Open | Existing timings predate the assembled normal route and must be remeasured there. |
| `>=70%` eligible deaths reach successor combat within two minutes | Open | Requires the real private cohort; automated reachability is not player behavior. |
| `>=80%` correctly explain the latest death | Open | Requires the real private-cohort comprehension check against stored death facts. |
| 10–20-person private cohort and operational release proof | Open | Hosting, backup/restore, support/telemetry operational review, and Steamworks evidence are not yet complete. |

## Release-candidate boundary

The optimized tester assembles the implemented player route and its server into one package. Packaging proves construction, content validation, CLI behavior, isolated launch health, and stale-artifact cleanup. It does not by itself prove the open normal-route, cohort, hosting, or platform gates.

## Current Next Step

`GB-M03-10` remains proven by its schema-correct hosted PostgreSQL least-privilege gate and [three-authority audit](GB-M03-10-audit.md). Hosted run [`30016103442`](https://github.com/MikeyPar/Gravebound/actions/runs/30016103442) applied schema 77, passed the real loot-source and four-family terminal-origin journeys, and cleared every mandatory suite through 25 item-lifecycle journeys. Those item journeys recorded login median `10.057 ms` / p95 `12.093 ms` and mutation median `6.892 ms` / p95 `9.818 ms`; they are not the Roadmap's 25 complete private loops. [Optimized tester r32](../evidence/GB-M03-tester-r32-manifest.md) packages exact runtime source `525b423`, protocol 1.25, and schema 78 after every production-blocking package gate and stale-`dist` cleanup passed. Hosted run [`30028182217`](https://github.com/MikeyPar/Gravebound/actions/runs/30028182217) proved live microrealm combat and isolated defeated-snapshot targeting; commit `2e8d2f4` fixes it with a green authoritative clear regression. Run [`30047477525`](https://github.com/MikeyPar/Gravebound/actions/runs/30047477525) received the exact live entry route/snapshot but reached QUIC idle timeout because the journey waited for a follow-up snapshot before its first activation input. Commit `1ad117e` primes ordinary movement from the verified entry snapshot and preserves full failure diagnostics. Commit `1f1d0fd` implements the remaining atomic TEL-003 source facts, and hosted run [`30048509525`](https://github.com/MikeyPar/Gravebound/actions/runs/30048509525) passes protocol/runtime authority plus schema-78 legacy upgrade/restart/replay and fresh PostgreSQL/redaction proof. The Current Next Step is exact-source route run [`30049060896`](https://github.com/MikeyPar/Gravebound/actions/runs/30049060896), then 25 complete ordinary loops, current aggregate timing, optimized Realm Gate capture, bounded operational telemetry proof, and the `03`/`03G`/`09` audits. The owner/operations gates for the 10–20-person cohort, comprehension metrics, provider backup/restore, deployment rollback, destination/privacy approval, and Steamworks evidence remain required for `14` and final M03 closure. Formal progress remains **15/23 (65%)**. Every action continues to be governed by `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`.
