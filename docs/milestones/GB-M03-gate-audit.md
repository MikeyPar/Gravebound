...
`GB-M03-10` remains proven by its schema-correct hosted PostgreSQL least-privilege gate and [three-authority audit](GB-M03-10-audit.md). The production-root source now covers durable Caldus extraction, server-planned placement, receipt-bound Hall return, ordinary lethal death, durable summary/Memorial/trace reads, successor creation, and checked Hall/danger control. Hosted run `29907197863` applied schemas 71/72 and passed both immutable loot-origin journeys. Schema 73 removes caller-authored terminal context by binding death, extraction, Recall, and successor outboxes to PostgreSQL-clock session origins and deriving session duration and item-power band from committed facts; run `29913145208` passed its hosted resolver before isolating a shared test-only `bytea` fixture defect. The identity transaction now creates the required Ash wallet under the account lock, while additive schema 75 corrects schema 74's content-label/namespace mismatch for only the missing pre-existing wipeable wallets. Optimized tester r24 packages source `33ee53d`, both executables, and migrations through 75 after all launch-mode smoke gates, with all older `dist` artifacts removed. The Current Next Step is hosted proof for all four schema-73 terminal families, schema 75, and the complete extraction/death/successor route, then the remaining `TEL-003` boss, party/contribution, and detailed network-health facts. After one route is green, run 25 loops and current aggregate timing, then publish the `03`, `03G`, and `09` audits. The owner/operations gates for the 10–20-person cohort, comprehension metrics, provider backup/restore, deployment rollback, and Steamworks evidence remain required for `14` and final M03 closure. Every action continues to be governed by `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`.
