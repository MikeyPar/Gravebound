...
`GB-M03-10` remains proven by its schema-correct hosted PostgreSQL least-privilege gate and [three-authority audit](GB-M03-10-audit.md). Hosted run [`30016103442`](https://github.com/MikeyPar/Gravebound/actions/runs/30016103442) applied schema 77 and passed every mandatory suite through 25 item-lifecycle journeys; those are not the Roadmap's 25 complete private loops. [Optimized tester r33](../evidence/GB-M03-tester-r33-manifest.md) packages source `0afc90e`, protocol 1.25, and schema 79, but its Docker-only primary launcher is not the intended self-contained tester boundary. Hosted run [`30053995319`](https://github.com/MikeyPar/Gravebound/actions/runs/30053995319) passes schema-79/pre-checkpoint proof and the exact B0 snapshot, then proves the production terminal owner drops B1's first authoritative frame at route version 9/tick 1243. The public route correctly remains `RoomSpawnWarning`; this is not a timeout to weaken. Commit `0698737` emits the complete owner error immediately, and focused run [`30066885083`](https://github.com/MikeyPar/Gravebound/actions/runs/30066885083) is resolving the exact substep. The release correction packages pinned PostgreSQL 17.10 on loopback and smoke-checks persistent-server readiness without Docker before replacing r33. The exact-25 evidence compiler exists but has not yet executed 25 complete private loops. The Current Next Step is the smallest evidence-backed terminal-owner fix, hosted green single-route proof, self-contained r34 publication, 25 ordinary full loops, current aggregate timing, optimized Realm Gate capture, bounded operational telemetry proof, and the `03`/`03G`/`09` audits. The owner/operations gates for the 10–20-person cohort, comprehension metrics, provider backup/restore, deployment rollback, destination/privacy approval, and Steamworks evidence remain required for `14` and final M03 closure. Formal progress remains **15/23 (65%)**. Every action continues to be governed by `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`.
