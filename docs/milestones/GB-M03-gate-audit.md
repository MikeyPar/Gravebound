...
`GB-M03-10` remains proven by its schema-correct hosted PostgreSQL least-privilege gate and [three-authority audit](GB-M03-10-audit.md). The production-root source reaches a durable Caldus extraction, validates server-planned reward placement, and returns by stored receipt to controllable Hall. Hosted run `29906346469` isolated schema 71's exact integer-literal projection defect, and additive schema 72 now delegates that call to the original immutable authority; negotiated crash collection and explicit disabled-worker ownership remain gameplay-independent. The Current Next Step is hosted schema-71/72 plus complete-extraction execution, then terminal telemetry origins, ordinary death/successor, 25 loops, and current login timing. After those engineering gates pass, publish the `03`, `03G`, and `09` audits. The owner/operations gates for the 10–20-person cohort, comprehension metrics, provider backup/restore, deployment rollback, and Steamworks evidence remain required for `14` and final M03 closure. Every action continues to be governed by `Gravebound_Production_GDD_v1_Canonical.md`, `Gravebound_Content_Production_Spec_v1.md`, and `Gravebound_Development_Roadmap_v1.md`.
