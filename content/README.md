...
`core_dev/oaths_bargains.json` is the independently hashed `GB-M03-05A` target. It binds the exact two Grave Arbalist Oaths, three Core Bargains, approved capability/effect tags, canonical English copy and initial warning, plus five editable choice icons under one immutable `core-dev.blake3.*` revision. Validate it with `cargo run --locked -p tools_content -- validate-core-oaths-bargains`. It contains no shrine, selection state, persistence, Ash mutation, runtime mechanic, release manifest, or route activation.

`core_dev/death_view.json` is the transitive, non-promotable `GB-M03-06D` presentation target. It pins its own records, dependency-backed assets, and canonical English copy plus the exact identity, world, item, encounter, Sir Caldus, Oath, and Bargain revisions reachable from a stored Core death. Field-typed resolution rejects cross-domain IDs and treats each enabled combat pattern as its canonical attack identity; missing or fabricated copy fails closed. Validate it with `cargo run --locked -p tools_content -- validate-core-death-view`. It contains no death mutation, successor command, commercial surface, route activation, or release/promotion operation.

`core_dev/successor_recovery.json` is the transitive, non-promotable `GB-M03-07` native recovery-presentation target. It pins the exact Core class/base silhouette, Lantern Halls authority, durable death catalog, item revision, and a closed ordered `en-US` copy set for the two-confirmation successor handoff. Validate it with `cargo run --locked -p tools_content -- validate-core-successor-recovery`. It contains no successor mutation, item grant, route admission, commerce surface, or release/promotion operation.
