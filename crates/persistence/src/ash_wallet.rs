...
fn resolve(
    ...
    let (code, after_balance) = match (request.kind, candidate) {
        ...
        (AshMutationKind::Spend, Some(value)) if value >= 0 => (AshMutationCode::Accepted, value),
...
async fn load_result(
    ...
    let row = sqlx::query(
        "SELECT mutation_id, payload_hash, mutation_kind, requested_amount, result_code, \
...
async fn insert_result(
    ...
    sqlx::query(
        "INSERT INTO ash_mutation_results \
         (namespace_id, account_id, mutation_id, payload_hash, mutation_kind, reason_code, \
...
mod tests {
    ...
    fn request(kind: AshMutationKind, amount: i32) -> AshMutationRequest {
        AshMutationRequest {
            account_id: [1; 16],
            mutation_id: [2; 16],
            payload_hash: [3; 32],
    ...
    fn earn_spend_cap_and_insufficient_results_are_exact() {
        let earned = resolve(
            &request(AshMutationKind::Earn, 40),
            StoredAshWallet {
                balance: 10,
                wallet_version: 2,
            },
        ...
        let spent = resolve(
            &request(AshMutationKind::Spend, 40),
            StoredAshWallet {
                balance: 50,
                wallet_version: 3,
            },
        ...
        let capped = resolve(
            &request(AshMutationKind::Earn, 1),
            StoredAshWallet {
        ...
        let insufficient = resolve(
            &request(AshMutationKind::Spend, 11),
            StoredAshWallet {
                balance: 10,
                wallet_version: 4,
            },
    ...
    fn malformed_requests_and_stored_results_fail_closed() {
        let mut malformed = request(AshMutationKind::Earn, 1);
        ...
        let mut result = resolve(
            &request(AshMutationKind::Earn, 5),
            StoredAshWallet {
                balance: 1,
                wallet_version: 1,
            },
