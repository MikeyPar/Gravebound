...
pub use production_extraction_intent::{
    ...
    PRODUCTION_EXTRACTION_INTENT_FRAME_SCHEMA_VERSION_V1, ProductionExtractionCoreRouteRevisionV1,
...
pub enum PersistenceError {
    ...
    #[error("stored production extraction intent violates the durable acceptance contract")]
    CorruptStoredProductionExtractionIntent,
    #[error("production extraction intent does not match the current selected actor authority")]
    ProductionExtractionIntentAuthorityMismatch,
    #[error("private-life bootstrap account does not exist")]
...
mod tests {
    ...
    fn production_extraction_intent_is_replay_first_and_conflict_audited() {
        ...
        for required in [
            ...
            "accepted_post_route_state_version = accepted_pre_route_state_version + 1",
            "route_records_blake3",
            "world_records_blake3",
            "canonical_attempt_hash",
            "commit_request_hash",
            "attempt_payload",
            "private_route_generation_allocations_v1",
            "prevent_production_extraction_intent_history_mutation_v1",
            "Reliable transport",
            "sequence is intentionally absent",
            "schema-62 binary",
            "Never rewrite",
        ] {
            assert!(migration.contains(required), "migration omitted {required}");
    ...
    fn core_echo_power_and_presentation_repair_is_exact_and_forward_only() {
        ...
        for required in [
            ...
            "Gravebound_Content_Production_Spec_v1.md",
            "Gravebound_Development_Roadmap_v1.md",
            "SPEC-CONFLICT-004-m03-core-identity.md",
            "CONT-ECHO-001",
            "pg_get_functiondef",
            "public.enforce_complete_death_graph_v1()",
            "WHEN 3 THEN 20 WHEN 4 THEN 30 WHEN 5 THEN 30 END",
            "sprite.class.grave_arbalist",
            "occurrence_count <> 4",
            "immutable legacy Core Echo records",
            "wipeable Core namespace",
            "published migration history must never be rewritten",
        ] {
            assert!(migration.contains(required), "migration omitted {required}");
        ...
        for prohibited in ["DROP TABLE", "TRUNCATE", "DELETE FROM", "JSON", "JSONB"] {
            assert!(
                !migration.contains(prohibited),
                "Core Echo authority repair leaked {prohibited}"
            );
        }
    }
    ...
    #[test]
    fn identity_ash_wallet_invariant_is_additive_and_preserves_existing_wallets() {
        let migration =
            include_str!("../../../migrations/0074_m03_identity_ash_wallet_invariant_v1.sql");
        for required in [
            "INSERT INTO ash_wallets",
            "SELECT account.namespace_id, account.account_id, 0, 1",
            "account.namespace_id = 'core-dev'",
            "NOT EXISTS",
            "ON CONFLICT (namespace_id, account_id) DO NOTHING",
            "schema 74 repairs",
        ] {
            assert!(migration.contains(required), "schema 74 omitted {required}");
        }
        let lowercase = migration.to_ascii_lowercase();
        for forbidden in [
            "drop table",
            ...
            assert!(
                !lowercase.contains(forbidden),
                "schema 74 introduced destructive wallet mutation {forbidden}"
            );
        }
    }
    ...
    fn identity_ash_wallet_namespace_repair_is_additive_and_exact() {
        ...
        for required in [
            ...
            "INSERT INTO ash_wallets",
            "SELECT account.namespace_id, account.account_id, 0, 1",
            "account.namespace_id = 'test.core'",
            "NOT EXISTS",
            "ON CONFLICT (namespace_id, account_id) DO NOTHING",
            "schema 75 additively repairs",
        ] {
            assert!(migration.contains(required), "schema 75 omitted {required}");
        }
        let lowercase = migration.to_ascii_lowercase();
        for forbidden in [
            "drop table",
    ...
    #[test]
    fn loot_telemetry_strict_immutability_repair_is_forward_only_and_exact() {
        let migration =
            include_str!("../../../migrations/0077_m03_loot_telemetry_strict_immutability_v1.sql");
        for required in [
            "Published schema 0076",
            "CREATE OR REPLACE FUNCTION enforce_item_ledger_telemetry_immutability_v1()",
            "IF TG_OP = 'DELETE' THEN",
            "item-ledger telemetry source history is immutable",
            "item-ledger telemetry publication may advance exactly once",
            "item-ledger telemetry source payload is immutable",
            "schema 0077 is the safe production state",
            "guarded disposable-database reset",
        ] {
            assert!(migration.contains(required), "schema 77 omitted {required}");
        }
        let lowercase = migration.to_ascii_lowercase();
        for forbidden in [
            "drop table",
